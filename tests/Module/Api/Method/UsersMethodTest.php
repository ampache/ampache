...
class UsersMethodTest extends MockeryTestCase
{
    ...
    public function testHandleReturnsEmptyResult(int $apiVersion): void
    {
        ...
        $browse->shouldReceive('set_sort_order')->with('', ['id', 'ASC'])->once();
        $browse->shouldReceive('set_filter')->with('disabled', 0)->once();
    ...
    #[DataProvider(methodName: 'apiVersionProvider')]
    public function testHandleReturnsUsers(int $apiVersion): void
    {
        $gatekeeper = $this->mock(GatekeeperInterface::class);
        $response   = $this->mock(ResponseInterface::class);
        $output     = $this->mock(ApiOutputInterface::class);
        ...
        // disabled users must never be returned
        $browse->shouldReceive('set_filter')->with('disabled', 0)->once();
        $browse->shouldReceive('set_conditions')->with('')->once();
        $browse->shouldReceive('get_objects')
            ->withNoArgs()
        ...
        $output->shouldReceive('setOffset')->with($apiVersion, 42)->once();
        $output->shouldReceive('setLimit')->with($apiVersion, 1)->once();
