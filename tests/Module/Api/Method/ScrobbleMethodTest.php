...
/**
 * Matching the song runs through the Song::can_scrobble() database static, so only the guards ahead
 * of it can be exercised without a database fixture.
 */
class ScrobbleMethodTest extends MockeryTestCase
{
    ...
    {
        return [
            'no song' => [['artist' => 'a', 'album' => 'b'], 'song'],
            'no artist' => [['song' => 's', 'album' => 'b'], 'artist'],
            'no album' => [['song' => 's', 'artist' => 'a'], 'album'],
        ];
    }
    ...
    public function testHandleThrowsIfUserIsNotValid(int $apiVersion): void
    {
        $gatekeeper = $this->mock(GatekeeperInterface::class);
        $response   = $this->mock(ResponseInterface::class);
        $output     = $this->mock(ApiOutputInterface::class);
        ...
        $this->expectExceptionMessage((string) $userId);
