...
class GenresMethodTest extends MockeryTestCase
{
    ...
    #[DataProvider(methodName: 'apiVersionProvider')]
    public function testHandleReturnsGenres(int $apiVersion): void
    {
        $gatekeeper = $this->mock(GatekeeperInterface::class);
        $response   = $this->mock(ResponseInterface::class);
        $output     = $this->mock(ApiOutputInterface::class);
    ...
    #[DataProvider(methodName: 'apiVersionProvider')]
    public function testHandleUsesExactMatchFilter(int $apiVersion): void
    {
        $gatekeeper = $this->mock(GatekeeperInterface::class);
        $response   = $this->mock(ResponseInterface::class);
        $output     = $this->mock(ApiOutputInterface::class);
        ...
        // `exact=1` must switch the browse from a fuzzy to an exact match
        $browse->shouldReceive('set_api_filter')
            ->with('exact_match', 'some-genre')
            ->once();
        $browse->shouldReceive('set_conditions')
            ->with('')
            ->once();
