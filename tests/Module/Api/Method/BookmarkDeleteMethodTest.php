...
/**
 * Resolving the stored bookmark goes through the Bookmark::getBookmarks() database static, so only
 * the guards can be exercised without a database fixture.
 */
class BookmarkDeleteMethodTest extends MockeryTestCase
{
    private BookmarkRepositoryInterface|MockInterface|null $bookmarkRepository;
    ...
    #[DataProvider(methodName: 'apiVersionProvider')]
    public function testHandleThrowsIfItemIsNotFound(int $apiVersion): void
    {
        $gatekeeper = $this->mock(GatekeeperInterface::class);
        $response   = $this->mock(ResponseInterface::class);
        $output     = $this->mock(ApiOutputInterface::class);
