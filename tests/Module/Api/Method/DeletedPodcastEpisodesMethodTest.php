...
class DeletedPodcastEpisodesMethodTest extends MockeryTestCase
{
    private ConfigContainerInterface|MockInterface|null $configContainer;
    private DeletedPodcastEpisodeRepositoryInterface|MockInterface|null $deletedPodcastEpisodeRepository;
    private ?DeletedPodcastEpisodesMethod $subject;
    ...
    public function testHandleReturnsEmptyResult(int $apiVersion): void
    {
        ...
        $this->deletedPodcastEpisodeRepository->shouldReceive('findAll')
            ->withNoArgs()
            ->once()
    ...
    public function testHandleReturnsErrorIfDisabled(int $apiVersion): void
    {
        ...
        $output->shouldReceive('error')
            ->with(
                ...
                'Enable: podcast',
                DeletedPodcastEpisodesMethod::ACTION,
                'system'
            )
            ->once()
            ->andReturn($result);
    ...
    public function testHandleReturnsResult(int $apiVersion): void
    {
        ...
        $output->shouldReceive('setLimit')
            ->with($apiVersion, 0)
            ->once();
        $output->shouldReceive('deleted')
            ->with($apiVersion, 'podcast_episode', $items)
            ->once()
            ->andReturn($result);
    ...
    protected function setUp(): void
    {
        ...
        $this->deletedPodcastEpisodeRepository = $this->mock(DeletedPodcastEpisodeRepositoryInterface::class);

        $this->subject = new DeletedPodcastEpisodesMethod(
