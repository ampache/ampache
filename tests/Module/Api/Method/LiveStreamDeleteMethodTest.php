...
class LiveStreamDeleteMethodTest extends MockeryTestCase
{
    private LiveStreamRepositoryInterface|MockInterface|null $liveStreamRepository;
    private MockInterface|PrivilegeCheckerInterface|null $privilegeChecker;
    private ?LiveStreamDeleteMethod $subject;
    ...
    public function testHandleDeletesLiveStream(int $apiVersion): void
    {
        ...
        $liveStream = $this->mock(Live_Stream::class);
        ...
        $this->liveStreamRepository->shouldReceive('findById')
            ->with($objectId)
            ->once()
            ->andReturn($liveStream);
        $this->liveStreamRepository->shouldReceive('delete')
            ->with($liveStream)
            ->once();

        // the resolved api version must reach the output untouched
        $output->shouldReceive('success')
            ->with($apiVersion, 'Deleted live_stream: ' . $objectId)
            ->once()
            ->andReturn($result);
    ...
    #[DataProvider(methodName: 'apiVersionProvider')]
    public function testHandleThrowsIfLiveStreamIsNotFound(int $apiVersion): void
    {
        $gatekeeper = $this->mock(GatekeeperInterface::class);
        $response   = $this->mock(ResponseInterface::class);
        $output     = $this->mock(ApiOutputInterface::class);
    ...
    {
        $this->liveStreamRepository = $this->mock(LiveStreamRepositoryInterface::class);
        $this->privilegeChecker     = $this->mock(PrivilegeCheckerInterface::class);

        $this->subject = new LiveStreamDeleteMethod(
