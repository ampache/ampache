...
class UrlToSongMethodTest extends MockeryTestCase
{
    ...
    public function testHandleReturnsErrorIfUrlCannotBeParsed(int $apiVersion): void
    {
        ...
        $this->assertSame(
            ...
            $this->subject->handle(
                ...
                $response,
                $output,
                ['url' => 'not-a-stream-url', 'api_format' => 'json', 'auth' => 'some-auth'],
                $user,
                $apiVersion
            )
        );
    ...
    #[DataProvider(methodName: 'apiVersionProvider')]
    public function testHandleThrowsIfUrlIsMissing(int $apiVersion): void
    {
        $gatekeeper = $this->mock(GatekeeperInterface::class);
        $response   = $this->mock(ResponseInterface::class);
        $output     = $this->mock(ApiOutputInterface::class);
        ...
        $this->expectExceptionMessage(sprintf(T_('Bad Request: %s'), 'url'));
