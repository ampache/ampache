...
class LabelArtistsMethodTest extends MockeryTestCase
{
    ...
    /**
     * An unknown label is reported as an empty artist list rather than an error
     */
    #[DataProvider(methodName: 'apiVersionProvider')]
    public function testHandleReturnsEmptyIfLabelIsNotFound(int $apiVersion): void
    {
        ...
        $this->labelRepository->shouldReceive('findById')
            ->with(666)
            ->once()
            ->andReturnNull();
    ...
    public function testHandleReturnsResult(int $apiVersion): void
    {
        ...
        $output->shouldReceive('artists')
            ->with($apiVersion, $results, ['songs'], $user, 'some-auth')
            ->once()
            ->andReturn($result);
