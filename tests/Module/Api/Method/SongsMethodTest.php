...
class SongsMethodTest extends MockeryTestCase
{
    ...
    public function testHandleReturnsEmptyResult(int $apiVersion): void
    {
        ...
        $browse->shouldReceive('set_type')->with('song')->once();
        $browse->shouldReceive('set_sort_order')->with('', ['title', 'ASC'])->once();
        ...
        $browse->shouldReceive('set_filter')->with('enabled', 1)->once();
    ...
    #[DataProvider(methodName: 'apiVersionProvider')]
    public function testHandleReturnsSongs(int $apiVersion): void
    {
        $gatekeeper = $this->mock(GatekeeperInterface::class);
        $response   = $this->mock(ResponseInterface::class);
        $output     = $this->mock(ApiOutputInterface::class);
        ...
        $browse->shouldReceive('set_api_filter')->with('exact_match', 'some-song')->once();
        $browse->shouldReceive('set_api_filter')->with('add', '')->once();
        $browse->shouldReceive('set_api_filter')->with('update', '')->once();

        // disabled songs must never be returned
        $browse->shouldReceive('set_filter')->with('enabled', 1)->once();
        $browse->shouldReceive('set_conditions')->with('')->once();
        $browse->shouldReceive('get_objects')
            ->withNoArgs()
