...
/**
 * Creating the catalog goes through the Catalog::create() database static, so only the guards ahead
 * of it can be exercised without a database fixture.
 */
class CatalogAddMethodTest extends MockeryTestCase
{
    private MockInterface|PrivilegeCheckerInterface|null $privilegeChecker;
    private ?CatalogAddMethod $subject;
    ...
    {
        return [
            'no name' => [['path' => '/some/path'], 'name'],
            'no path' => [['name' => 'some-name'], 'path'],
        ];
    }

    #[DataProvider(methodName: 'apiVersionProvider')]
    public function testHandleReturnsErrorIfRemoteCatalogHasNoUsername(int $apiVersion): void
    {
    ...
    protected function setUp(): void
    {
        ...
        $this->subject = new CatalogAddMethod(
