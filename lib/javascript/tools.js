...
var currentEditId = 0;
//var currentRadioDJId = 0;
function loadUtilsData(formId, path, id) {

    var utilsURL = '//utils.gradio.local/admin/checkupload/path/t?id=' + id +'&path=';
    currentEditId = id;

    //$(formId).load(utilsURL + path);
    ...
    promise.done(function(response) {
        ...
        <table class="table search-results">
                <thead>
                  <tr>
                    <th></th>
                    <th class="check-size col-xs-1">Relevance</th>
                    <th class="check-tune col-xs-1">ID</th>
                    <th class="check-tune col-xs-4">Tune</th>
                    <th class="check-duration col-xs-1">Duration</th>
                    <th class="check-bitrate col-xs-1">Bitrate</th>
                    <th class="check-size col-xs-1">Size</th>
                    <th class="action col-xs-1">Action</th>
                  </tr>
                </thead>
                <tbody>
                </tbody>
        ...
        //
        $(formId).prepend(tableHTML);

        var template = $('#checkUploadResult').html();
            Mustache.parse(template);
            var rendered = Mustache.render(template, response);
            //console.debug(response);

            $(".search-results tbody").prepend(rendered);

            $.each(response.check.results, function(row, song) {
              if (typeof song === 'object') {
                $.each(song, function(key, value) {
                    $('.check-details-info-list[data-id="' + song.ID + '"][data-parentid="' + response.check.id + '"]')
    ...
    promise.fail(function() {
        $(formId).prepend('<p>Oh no, something went wrong!</p>');
    });
}

function mergeDBS(songId, radioDJId) {

    var utilsURL = '//utils.gradio.local/admin/checkupload/merge/t?ampache_song_id=' + songId +'&song_id=' + radioDJId;
    ...
    promise.done(function(response) {
        console.debug(response);
        $('.edit_dialog_content').prepend('<p>Merged!</p>');
    });

    promise.fail(function() {
        $('.edit_dialog_content').prepend('<p>Oh no, something went wrong!</p>');
    });
}

function showToRadioDBDialog(edit_type, edit_id, edit_form_id, edit_title, refresh_row_prefix) {
    var parent = this;
    parent.editFormId = 'form#' + edit_form_id;
    parent.contentUrl = jsAjaxServer + '/edit.server.php?action=show_edit_object&id=' + edit_id + '&type=' + edit_type;
    parent.saveUrl = jsAjaxServer + '/edit.server.php?action=edit_object&id=' + edit_id + '&type=' + edit_type;
    ...
    if (tag_choices == undefined && tag_choices != '') {
        ...
        $.ajax(jsAjaxServer + '/ajax.server.php?page=tag&action=get_tag_map', {
            success: function(data) {
                tag_choices = $(data).find('content').text();
                if (tag_choices != '') {
                    showToRadioDBDialog(edit_type, edit_id, edit_form_id, edit_title, refresh_row_prefix);
                }
            }, type: 'post', dataType: 'xml'
        });
        return;
    ...
    setTimeout(function(){
      //console.debug( $(parent.editFormId).data("path") );
      loadUtilsData(parent.editFormId, $(parent.editFormId).data("path"), edit_id);
    }, 1000);
    $('#editdialog').dialog('open');
