...
function check_inline_song_edit(type, song) {
    ...
    var target = type + '_select_song_' + song;

    if ($(source).options[$(source).selectedIndex].value == -1) {
        $(target).innerHTML = '<input type="text" name="' + type + '_name" value="New ' + type + '" onclick="this.select();" />';

        if (type == 'album') {
            $(target).innerHTML += '<label for="year">Year</label><input type="text" name="year" id="year" size="5" /><label for="disk">Disk #</label><input type="text" name="disk" id="disk" size="3" />';
        }
    } else {
        $(target).innerHTML = '';
    }
