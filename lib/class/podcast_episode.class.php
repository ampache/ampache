...
class Podcast_Episode extends database_object implements media, library_item
{
    ...
    public function check_play_history($user, $agent)
    {
        ...
        if ($previous['object_id'] == $this->id && $diff <= ($this->time + 5) && $diff > 0) {
            debug_event('podcast_episode.class', 'Repeated podcast_episode too quickly (' . $diff . 's), not recording stats for {' . $previous['object_id'] . '}', 3);
        ...
        if (($diff < $skiptime || ($diff < $skiptime && $previous['time'] > $skiptime)) && $diff > 0) {
            debug_event('podcast_episode.class', 'Last ' . $previous['object_type'] . ' played within skip limit (' . $diff . 's). Skipping {' . $previous['object_id'] . '}', 3);
            ...
            Useractivity::del_activity($previous['date'], 'podcast_episode', $previous['user']);
        }
