...
class Song extends database_object implements media, library_item
{
    ...
    public static function build_cache($song_ids, $limit_threshold = '')
    {
        ...
        $sql = 'SELECT `song`.`id`, `file`, `catalog`, `album`, ' .
            '`year`, `disk`, `artist`, `title`, `bitrate`, `rate`, ' .
    ...
    private function _get_info($limit_threshold = '')
    {
        ...
        $sql = 'SELECT `song`.`id`, `song`.`file`, `song`.`catalog`, `song`.`album`, `album`.`album_artist` AS `albumartist`, `song`.`year`, `song`.`disk`, `song`.`artist`,' .
    ...
    public static function update_song($song_id, Song $new_song)
    {
        ...
        Dba::write($sql, array($new_song->album, $new_song->year, $new_song ->disk, $new_song->artist, $new_song->title, $new_song->composer, $new_song->bitrate, $new_song->rate,
     ...
     * @param int $song_id
     */
    public static function update_disk($new_year, $song_id)
    {
        self::_update_item('disk', $new_disk, $song_id, 50, true);
    } // update_disk
