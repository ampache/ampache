...
<tr>
    ...
    if (count($favorite_artists)) {
        ...
        $title = '<a href="' . AmpConfig::get('web_path') . '/stream.php?action=play_favorite&amp;type=artist">' .
    ...
    if (count($favorite_albums)) {
        $items = $working_user->format_favorites($favorite_albums);
                $title = '<a href="' . AmpConfig::get('web_path') . '/stream.php?action=play_favorite&amp;type=album">' .
    ...
    if (count($favorite_songs)) {
        $items = $working_user->format_favorites($favorite_songs);
                $title = '<a href="' . AmpConfig::get('web_path') . '/stream.php?action=play_favorite&amp;type=song">' .
