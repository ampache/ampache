    ...
    if (!$user_authenticated) {
        ...
        if (AmpConfig::get('use_auth') && AmpConfig::get('require_session')) {
            if (!AmpConfig::get('require_localnet_session') && Access::check_network('network', Core::get_global('user')->id, '5')) {
                debug_event('play/index', 'Streaming access allowed for local network IP ' . filter_input(INPUT_SERVER, 'REMOTE_ADDR', FILTER_SANITIZE_STRING, FILTER_FLAG_NO_ENCODE_QUOTES), 4);
            } else {
...
if (AmpConfig::get('access_control')) {
    if (!Access::check_network('stream', Core::get_global('user')->id, '25') &&
        ...
        debug_event('play/index', "Streaming Access Denied: " . filter_input(INPUT_SERVER, 'REMOTE_ADDR', FILTER_SANITIZE_STRING, FILTER_FLAG_NO_ENCODE_QUOTES) . " does not have stream level access", 3);
        UI::access_denied();
...
if (AmpConfig::get('downsample_remote')) {
    if (!Access::check_network('network', Core::get_global('user')->id, '0')) {
        debug_event('play/index', 'Downsampling enabled for non-local address ' . filter_input(INPUT_SERVER, 'REMOTE_ADDR', FILTER_SANITIZE_STRING, FILTER_FLAG_NO_ENCODE_QUOTES), 5);
