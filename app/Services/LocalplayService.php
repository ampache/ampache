...
class LocalplayService
{
     ...
     * gui
     */
    public function _get_info($type = null)
    {
        if ($type != null) {
            $this->type = $type;
        }
    ...
    private function _load_player()
    {
        ...
        $filename = base_path('modules/localplay/') . $this->type . '/' . $this->type . '.controller.php';
        $include  = require_once $filename;

        if (!$include) {
            /* Throw Error Here */
            
            Log::error('localplay: Unable to load ' . $this->type . ' controller');

            return false;
        } // include
        else {
            $class_name    = '\\Modules\\Localplay\\Ampache' . ucfirst($this->type);
            $this->_player = new $class_name();
            if (!($this->_player instanceof Localplay_Controller)) {
                Log::error('Localplay: ' . $this->type . ' not an instance of controller abstract, unable to load');
                unset($this->_player);
...
//        $name = Ajax::text('?page=localplay&action=command&command=skip&id=' . $id, $name, 'localplay_skip_' . $id);
    ...
    {
        /* First open the dir */
        $basedir = base_path('/modules/localplay');
        $handle  = opendir($basedir);

        if (!is_resource($handle)) {
            Log::error('Localplay Error: Unable to read localplay controller directory');
        ...
        while (false !== ($file = readdir($handle))) {
            ...
            if (! file_exists($basedir . '/' . $file . '/' . $file . '.controller.php')) {
                Log::error('Localplay: Missing class for ' . $file);
                continue;
            }
    ...
    public static function is_enabled($controller)
    {
        ...
        $localplay = new LocalplayService($controller);
    ...
    public function install()
    {
        ...
        PreferenceService::update('allow_localplay_playback', '-1', '1');
        PreferenceService::update('localplay_level', auth()->user()->id, '100');
        PreferenceService::update('localplay_controller', auth()->user()->id, $this->_player->get_type());
    ...
    public function uninstall()
    {
        ...
        if (AmpConfig::get('localplay_controller') == $this->type) {
            PreferenceService::update('localplay_controller', auth()->user()->id, '');
        }
    ...
    public function connect()
    {
        if (!$this->_player->connect()) {
            Log::error('Localplay Error: Unable to connect, check ' . $this->type . ' controller');
    ...
    public function stop()
    {
        if (!$this->_player->stop()) {
            Log::error('Localplay Error: Unable to stop playback, check ' . $this->type . ' controller');
    ...
    {
        Log::error('localplay: Deprecated add method called: ' . json_encode($object));
    ...
    public function add_url(Stream_URL $url)
    {
        if (!$this->_player->add_url($url)) {
            Log::error('Localplay: Unable to add url ' . $url . ', check ' . $this->type . ' controller');
    ...
    public function repeat($state)
    {
        ...
        if (!$data) {
            Log::error("Localplay Error Unable to set Repeat to $state");
        }
    ...
    public function random($state)
    {
        ...
        if (!$data) {
            Log::error("Localplay Error Unable to set Random to $state");
        }
    ...
    public function status()
    {
        ...
        if (!count($data)) {
            Log::Error('Localplay Error Unable to get status, check ' . $this->type . ' controller');
    ...
    public function get()
    {
        ...
        if (!count($data) or !is_array($data)) {
            Log::error('Localplay Error Unable to get song info, check ' . $this->type . ' controller');
    ...
    public function volume_set($value)
    {
        ...
        if (!$this->_player->volume($value)) {
            Log::error('Localplay Error: Unable to set volume, check ' . $this->type . ' controller');
    ...
    public function volume_up()
    {
        if (!$this->_player->volume_up()) {
            Log::error('Localplay Error: Unable to increase volume, check ' . $this->type . ' controller');
    ...
    public function volume_down()
    {
        if (!$this->_player->volume_down()) {
            Log::error('Localplay Error: Unable to decrease volume, check ' . $this->type . ' controller');
    ...
    public function volume_mute()
    {
        if (!$this->_player->volume(0)) {
            Log::error('Localplay Error: Unable to mute volume, check ' . $this->type . ' controller');
    ...
    public function skip($track_id)
    {
        if (!$this->_player->skip($track_id)) {
            Log::error('Localplay Error: Unable to skip to next song, check ' . $this->type . ' controller');
    ...
    public function prev()
    {
        if (!$this->_player->prev()) {
            Log::error('Localplay Error: Unable to skip to previous song, check ' . $this->type . ' controller');
    ...
    public function pause()
    {
        if (!$this->_player->pause()) {
            Log::error('Localplay error: Unable to pause song, check ' . $this->type . ' controller');
    ...
    public function delete_track($object_id)
    {
        if (!$this->_player->delete_track($object_id)) {
            Log::error('localplay Error: Unable to remove songs, check ' . $this->type . ' controller');
    ...
    public function delete_all()
    {
        if (!$this->_player->clear_playlist()) {
            Log::error('localplay Error: Unable to delete entire playlist, check ' . $this->type . ' controller');
