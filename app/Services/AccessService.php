...
class AccessService
{
    ...
    private function _verify_range($startp, $endp)
    {
        ...
        if (!$startn && $startp != '0.0.0.0' && $startp != '::') {
            $this->AmpError->add('start', T_('Invalid IPv4 / IPv6 Address Entered'));
            
            return false;
        }
        if (!$endn) {
            $this->AmpError::add('end', T_('Invalid IPv4 / IPv6 Address Entered'));
        }
        
        if (strlen(bin2hex($startn)) != strlen(bin2hex($endn))) {
            $this->AmpError::add('start', T_('IP Address Version Mismatch'));
            $this->AmpError::add('end', T_('IP Address Version Mismatch'));
    ...
    public function create(array $data)
    {
        ...
        if (self::exists($data)) {
            ...
            $this->AmpError::add('general', T_('Duplicate ACL defined'));
     ...
     * @return boolean
     */
    public function check_function($type)
    {
        switch ($type) {
            case 'download':
     ...
     * @return boolean
     */
    public function check_network($type, $user=null, $level, $ip=null, $apikey = null)
    {
     ...
     * @return array
     */
    public function get_access_lists()
    {
        $sql        = 'SELECT `id` FROM `access_list`';
