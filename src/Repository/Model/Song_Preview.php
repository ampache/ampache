...
class Song_Preview extends database_object implements Media, displayable_item, container_item
{
    ...
    public function get_f_album_link(): ?string
    {
        if ($this->f_album_link === null && $this->f_album !== null) {
            $this->f_album_link = "<a href=\"" . AmpConfig::get_web_path() . "/albums.php?action=show_missing&mbid=" . rawurlencode((string) $this->album_mbid) . "&artist=" . $this->artist . "\" title=\"" . scrub_out($this->f_album) . "\">" . scrub_out($this->f_album) . "</a>";
        }
