...
/*
        ...
        foreach ($results as $result) {
            ...
            if (strlen((string)$image[0]['raw']) > '5') {
                $inserted = $art->insert($image[0]['raw'], $image[0]['mime']);
                // If they've enabled resizing of images generate a thumbnail
                if (AmpConfig::get('resize_images', false)) {
                    $size  = array('width' => 275, 'height' => 275);
                    $thumb = $art->generate_thumb($image['raw'], $size, $image['mime']);
                    if (!empty($thumb)) {
        ...
        switch ($type) {
            ...
            case 'artist':
                foreach ($libitem->get_child_ids() as $album_id) {
                    ...
                    static::getAlbumRepository()->updateTime($album);
                }
