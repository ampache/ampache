...
abstract class Catalog extends database_object
{
    ...
    public static function update_song_from_tags($results, Song $song)
    {
        ...
        if (!empty($artists_array)) {
            debug_event(self::class, "artist_array: " . print_r($artists_array, true), 4);
        ...
        foreach ($artist_mbid_array as $song_artist_mbid) {
            ...
            if ($song_artist_id > 0) {
                ...
                if (!in_array($song_artist_id, $song_artist_maps)) {
                    $song_artist_maps[] = $song_artist_id;
        ...
        foreach ($albumartist_mbid_array as $album_artist_mbid) {
            ...
            if ($album_artist_id > 0) {
                ...
                if (!in_array($album_artist_id, $album_artist_maps)) {
                    $album_artist_maps[] = $album_artist_id;
    ...
    public static function update_counts()
    {
        ...
        // artist.time
        $sql = "UPDATE `artist`, (SELECT SUM(`song`.`time`) as `time`, `song`.`artist` FROM `song` LEFT JOIN `artist_map` ON `song`.`id` = `artist_map`.`object_id` AND `song`.`artist` = `artist_map`.`artist_id` AND `artist_map`.`object_type` = 'song' GROUP BY `song`.`artist`) AS `song` SET `artist`.`time` = `song`.`time` WHERE `artist`.`id` = `song`.`artist` AND (`artist`.`time` != `song`.`time` OR `artist`.`time` IS NULL);";
        Dba::write($sql);
        // artist.total_count
        $sql = "UPDATE `artist`, (SELECT COUNT(DISTINCT CONCAT(COALESCE(`album`.`prefix`, ''), `album`.`name`, COALESCE(`album`.`album_artist`, ''), COALESCE(`album`.`mbid`, ''), COALESCE(`album`.`year`, ''))) AS `album_group_count`, `album_artist` FROM `album` LEFT JOIN `catalog` ON `catalog`.`id` = `album`.`catalog` LEFT JOIN `artist_map` ON `album`.`id` = `artist_map`.`object_id` AND `album`.`album_artist` = `artist_map`.`artist_id` AND `artist_map`.`object_type` = 'album' WHERE `catalog`.`enabled` = '1' GROUP BY `album_artist`) AS `album` SET `artist`.`album_group_count` = `album`.`album_group_count` WHERE `artist`.`id` = `album`.`album_artist`;";
        Dba::write($sql);
        // artist.album_count
        $sql = "UPDATE `artist`, (SELECT COUNT(DISTINCT `album`.`id`) AS `album_count`, `album_artist` FROM `album` LEFT JOIN `catalog` ON `catalog`.`id` = `album`.`catalog` LEFT JOIN `artist_map` ON `album`.`id` = `artist_map`.`object_id` AND `album`.`album_artist` = `artist_map`.`artist_id` AND `artist_map`.`object_type` = 'album' WHERE `catalog`.`enabled` = '1' GROUP BY `album_artist`) AS `album` SET `artist`.`album_count` = `album`.`album_count` WHERE `artist`.`album_count` != `album`.`album_count` AND `artist`.`id` = `album`.`album_artist`;";
        Dba::write($sql);
        ...
        Dba::write($sql);
        // artist.song_count
        $sql = "UPDATE `artist`, (SELECT COUNT(`song`.`id`) AS `song_count`, `artist` FROM `song` LEFT JOIN `catalog` ON `catalog`.`id` = `song`.`catalog` LEFT JOIN `artist_map` ON `song`.`id` = `artist_map`.`object_id` AND `song`.`artist` = `artist_map`.`artist_id` AND `artist_map`.`object_type` = 'song' WHERE `catalog`.`enabled` = '1' GROUP BY `song`.`artist`) AS `song` SET `artist`.`song_count` = `song`.`song_count` WHERE `artist`.`song_count` != `song`.`song_count` AND `artist`.`id` = `song`.`artist`;";
        Dba::write($sql);
    ...
    public static function update_mapping($table)
    {
        ...
        if ($table == 'artist') {
            $sql = "INSERT IGNORE INTO `catalog_map` (`catalog_id`, `object_type`, `object_id`) SELECT `song`.`catalog`, 'artist', `artist`.`id` FROM `artist` LEFT JOIN `song` ON `song`.`artist` = `artist`.`id` WHERE `song`.`catalog` > 0 GROUP BY `song`.`catalog`, 'artist', `artist`.`id` UNION SELECT `album`.`catalog`, 'artist', `artist`.`id` FROM `artist` LEFT JOIN `album` ON `album`.`album_artist` = `artist`.`id` WHERE `album`.`catalog` > 0 GROUP BY `album`.`catalog`, 'artist', `artist`.`id`;";
            Dba::write($sql);
        } else {
