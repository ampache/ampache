...
class Artist extends database_object implements library_item, GarbageCollectibleInterface
{
    ...
    public static function update_artist_counts($artist_id)
    {
        if ($artist_id > 0) {
            ...
            Dba::write($sql, $params);
            // artist.album_count
            $sql = "UPDATE `artist`, (SELECT COUNT(DISTINCT `album`.`id`) AS `album_count` FROM `artist_map` LEFT JOIN `album` ON `album`.`id` = `artist_map`.`object_id` AND `artist_map`.`object_type` = 'album' LEFT JOIN `catalog` ON `catalog`.`id` = `album`.`catalog` WHERE `artist_map`.`artist_id` = ? AND `catalog`.`enabled` = '1') AS `album` SET `artist`.`album_count` = `album`.`album_count` WHERE `artist`.`id` = `album`.`album_artist`;";
            Dba::write($sql, $params);
