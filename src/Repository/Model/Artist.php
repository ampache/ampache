...
class Artist extends database_object implements library_item, GarbageCollectibleInterface
{
    ...
    public static function update_artist_counts()
    {
        ...
        Dba::write($sql);
        // artist.total_count
        $sql = "UPDATE `artist`, (SELECT DISTINCT COUNT(`object_count`.`object_id`) AS `total_count`, `artist_map`.`artist_id` FROM `object_count` LEFT JOIN `artist_map` ON `artist_map`.`object_id` = `object_count`.`object_id` AND `object_count`.`count_type` = 'stream' LEFT JOIN `song` ON `object_count`.`object_id` = `song`.`id` LEFT JOIN `artist_map` AS `album_artist` ON `album_artist`.`object_id` = song.album AND `album_artist`.`object_id` = `artist_map`.`artist_id` WHERE `object_count`.`object_type` = 'song' AND `object_count`.`count_type` = 'stream' GROUP BY `artist_map`.`artist_id`) AS `object_count` SET `artist`.`total_count` = `object_count`.`total_count` WHERE `artist`.`total_count` != `object_count`.`total_count` AND `artist`.`id` = `object_count`.`artist_id`;";
        Dba::write($sql);
        // object count = 0
