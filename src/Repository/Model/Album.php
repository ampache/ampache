...
class Album extends database_object implements library_item, CatalogItemInterface
{
    ...
    private function _get_extra_info(): array
    {
        ...
        if (
            ...
            $sql        = "SELECT `artist`.`name` AS `artist_name`, `artist`.`prefix` AS `artist_prefix`, `song`.`artist` AS `album_artist` FROM `song` INNER JOIN `album` ON `album`.`id`=`song`.`album` INNER JOIN `artist` ON `artist`.`id`=`album`.`album_artist` WHERE `song`.`album` = ? GROUP BY `song`.`album`, `artist`.`prefix`, `artist`.`name`, `song`.`artist`;";
