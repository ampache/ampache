    ...
    public function update(array $data)
    {
        ...
        if ($album_id > 0 && $album_id != $this->id) {
            ...
            if (!empty($release_type) && $release_type != $this->release_type) {
                ...
                $ndata['release_type']   = $release_type;
                $changed['release_type'] = 'release_type';
            }
            if (!empty($release_status) && $release_status != $this->release_status) {
                self::update_field('release_status', $release_status, $this->id);
                $ndata['release_status']   = $release_status;
                $changed['release_status'] = 'release_status';
            }
            if (!empty($catalog_number) && $catalog_number != $this->catalog_number) {
                self::update_field('catalog_number', $catalog_number, $this->id);
                $ndata['catalog_number']   = $catalog_number;
                $changed['catalog_number'] = 'catalog_number';
            }
            if (!empty($barcode) && $barcode != $this->barcode) {
                self::update_field('barcode', $barcode, $this->id);
                $ndata['barcode']   = $barcode;
                $changed['barcode'] = "barcode";
            }
