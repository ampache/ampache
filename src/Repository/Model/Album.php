...
class Album extends database_object implements library_item, CatalogItemInterface
{
    ...
    public static function update_album_count(int $album_id): void
    {
        ...
        Dba::write($sql, $params);
        // song.weight
        UPDATE `song` LEFT JOIN (SELECT `object_id`, COUNT(*) AS `rating_count` FROM `rating`  WHERE `object_type` = 'song' GROUP BY `object_id`) `rating` ON `song`.`id` = `rating`.`object_id` LEFT JOIN (SELECT `object_id`, COUNT(*) AS `flag_count` FROM `user_flag` WHERE `object_type` = 'song' GROUP BY `object_id`) `flag` ON `song`.`id` = `flag`.`object_id` SET `song`.`weight` = COALESCE(`rating`.`rating_count`, 0) + COALESCE(`flag`.`flag_count`, 0) + (COALESCE(`song`.`total_count`, 0) - COALESCE(`song`.`total_skip`, 0));
// album.weight
UPDATE `album` LEFT JOIN (SELECT `object_id`, COUNT(*) AS `rating_count` FROM `rating` WHERE `object_type` = 'album' GROUP BY `object_id`) `rating` ON `album`.`id` = `rating`.`object_id` LEFT JOIN (SELECT `object_id`, COUNT(*) AS `flag_count` FROM `user_flag` WHERE `object_type` = 'album' GROUP BY `object_id`) `flag` ON `album`.`id` = `flag`.`object_id` SET `album`.`weight` = COALESCE(`rating`.`rating_count`, 0) + COALESCE(`flag`.`flag_count`, 0) + (COALESCE(`album`.`total_count`, 0) - COALESCE(`album`.`total_skip`, 0));
// artist.weight
UPDATE `artist` LEFT JOIN (SELECT `object_id`, COUNT(*) AS `rating_count` FROM `rating` WHERE `object_type` = 'artist' GROUP BY `object_id`) `rating` ON `artist`.`id` = `rating`.`object_id` LEFT JOIN ( SELECT `object_id`, COUNT(*) AS `flag_count` FROM `user_flag` WHERE `object_type` = 'artist' GROUP BY `object_id` ) `flag` ON `artist`.`id` = `flag`.`object_id` SET `artist`.`weight` = COALESCE(`rating`.`rating_count`, 0) + COALESCE(`flag`.`flag_count`, 0) + (COALESCE(`artist`.`total_count`, 0) - COALESCE(`artist`.`total_skip`, 0));
// podcast.weight
UPDATE `podcast` LEFT JOIN (SELECT `object_id`, COUNT(*) AS `rating_count` FROM `rating` WHERE `object_type` = 'podcast' GROUP BY `object_id`) `rating` ON `podcast`.`id` = `rating`.`object_id` LEFT JOIN ( SELECT `object_id`, COUNT(*) AS `flag_count` FROM `user_flag` WHERE `object_type` = 'podcast' GROUP BY `object_id` ) `flag` ON `podcast`.`id` = `flag`.`object_id` SET `podcast`.`weight` = COALESCE(`rating`.`rating_count`, 0) + COALESCE(`flag`.`flag_count`, 0) + (COALESCE(`podcast`.`total_count`, 0) - COALESCE(`podcast`.`total_skip`, 0));
// podcast_episode.weight
UPDATE `podcast_episode` LEFT JOIN (SELECT `object_id`, COUNT(*) AS `rating_count` FROM `rating` WHERE `object_type` = 'podcast_episode' GROUP BY `object_id`) `rating` ON `podcast_episode`.`id` = `rating`.`object_id` LEFT JOIN ( SELECT `object_id`, COUNT(*) AS `flag_count` FROM `user_flag` WHERE `object_type` = 'podcast_episode' GROUP BY `object_id` ) `flag` ON `podcast_episode`.`id` = `flag`.`object_id` SET `podcast_episode`.`weight` = COALESCE(`rating`.`rating_count`, 0) + COALESCE(`flag`.`flag_count`, 0) + (COALESCE(`podcast_episode`.`total_count`, 0) - COALESCE(`podcast_episode`.`total_skip`, 0));
// video.weight
UPDATE `video` LEFT JOIN (SELECT `object_id`, COUNT(*) AS `rating_count` FROM `rating` WHERE `object_type` = 'video' GROUP BY `object_id`) `rating` ON `video`.`id` = `rating`.`object_id` LEFT JOIN ( SELECT `object_id`, COUNT(*) AS `flag_count` FROM `user_flag` WHERE `object_type` = 'video' GROUP BY `object_id` ) `flag` ON `video`.`id` = `flag`.`object_id` SET `video`.`weight` = COALESCE(`rating`.`rating_count`, 0) + COALESCE(`flag`.`flag_count`, 0) + (COALESCE(`video`.`total_count`, 0) - COALESCE(`video`.`total_skip`, 0));
