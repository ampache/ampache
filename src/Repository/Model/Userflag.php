...
class Userflag extends database_object
{
    ...
    public static function get_latest_sql($type, $user_id = null)
    {
        ...
        $sql .= ($allow_group_disks)
            ? " GROUP BY `album`.`prefix`, `album`.`name`,user `album`.`album_artist`, `album`.`release_type`, `album`.`release_status`, `album`.`mbid`, `album`.`year`, `album`.`original_year` ORDER BY `count` DESC, `date` DESC "
