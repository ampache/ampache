        ...
        foreach ($this->rules as $rule) {
            ...
            switch ($rule[0]) {
                ...
                case 'possible_duplicate_album':
                    $where[]                     = "(`dupe_album_search1`.`dupe_album_id1` IS NOT NULL OR `dupe_album_search2`.`dupe_album_id2` IS NOT NULL) OR `mbid_group` IN (SELECT `mbid_group` FROM `album` WHERE `disk` = 1 GROUP BY `mbid_group`, `disk` HAVING COUNT(`mbid_group`) > 1)";
        ...
        foreach ($this->rules as $rule) {
            ...
            switch ($rule[0]) {
                ...
                case 'artist':
                    ...
                    break;
                case 'album_artist':
                    $where[]         = "(`album_artist`.`name` $sql_match_operator ? " .
                        " OR LTRIM(CONCAT(COALESCE(`album_artist`.`prefix`, ''), " .
                        "' ', `album_artist`.`name`)) $sql_match_operator ?)";
                    $parameters[]          = $input;
                    $parameters[]          = $input;
                    $table['album']        = "LEFT JOIN `album` ON `song`.`album`=`album`.`id`";
