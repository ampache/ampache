...
class Art extends database_object
{
     ...
     */
    private function test_image($source): bool
    {
    ...
    public static function url($uid, $type, $sid = null, $thumb = null): ?string
    {
        ...
        if (AmpConfig::get('stream_beautiful_url')) {
            ...
            $actionStr = ($type === 'user')
                    ...
                    : '';
            $url = AmpConfig::get('web_path') . '/client/image.php?' . $actionStr . 'object_id=' . $uid . '&object_type=' . scrub_out($type);
            if ($thumb !== null) {
                $url .= '&thumb=' . $thumb;
            }
    ...
    public static function display(
        ...
        if ($link === null) {
            $link = AmpConfig::get('web_path') . "/client/image.php?object_id=" . $object_id . "&object_type=" . $object_type . "&thumb=" . $thumb;
            if (AmpConfig::get('use_auth') && AmpConfig::get('require_session')) {
        ...
        echo ">";
        $imgurl = AmpConfig::get('web_path') . "/client/image.php?object_id=" . $object_id . "&object_type=" . $object_type . "&thumb=" . $thumb;
        if ($kind != 'default') {
            $imgurl .= '&kind=' . $kind;
        }
        ...
        if ($prettyPhoto) {
            ...
            if (
                ...
                echo "<a href=\"javascript:NavigateTo('" . AmpConfig::get('web_path') . "/client/arts.php?action=show_art_dlg&object_type=" . $object_type . "&object_id=" . $object_id . "&burl=' + getCurrentPage());\">";
                echo Ui::get_material_symbol('edit', T_('Edit/Find Art'));
                echo "</a>";
                echo "<a href=\"javascript:NavigateTo('" . AmpConfig::get('web_path') . "/client/arts.php?action=clear_art&object_type=" . $object_type . "&object_id=" . $object_id . "&burl=' + getCurrentPage());\" onclick=\"return confirm('" . T_('Do you really want to reset art?') . "');\">";
