...
final readonly class ArtistRepository implements ArtistRepositoryInterface
{
    ...
    public function setChildrenEnabled(int $artistId, bool $enabled): void
    {
        ...
        // the albums that just changed carry a stored song count, and it is what every page reads: one pass
        // over the same map rather than a call per album
        $this->connection->query(
            "UPDATE `album` INNER JOIN `artist_map` ON `artist_map`.`object_id` = `album`.`id` AND `artist_map`.`object_type` = 'album' SET `album`.`song_count` = (SELECT COUNT(`song`.`id`) FROM `song` LEFT JOIN `catalog` ON `catalog`.`id` = `song`.`catalog` WHERE `song`.`album` = `album`.`id` AND `catalog`.`enabled` = '1' AND `song`.`enabled` = 1) WHERE `artist_map`.`artist_id` = ?",
            [$artistId]
        );
    }
    ...
    public function updateAllCounts(): void
    {
        ...
        $statements = [
            ...
            "UPDATE `artist`, (SELECT COUNT(DISTINCT `album`.`id`) AS `album_count`, `artist_map`.`artist_id` FROM `artist_map` LEFT JOIN `album` ON `album`.`id` = `artist_map`.`object_id` AND `artist_map`.`object_type` = 'album' LEFT JOIN `catalog` ON `catalog`.`id` = `album`.`catalog` WHERE `catalog`.`enabled` = '1' AND `album`.`enabled` = 1 GROUP BY `artist_map`.`artist_id`) AS `album` SET `artist`.`album_count` = `album`.`album_count` WHERE `artist`.`album_count` != `album`.`album_count` AND `artist`.`id` = `album`.`artist_id`;",
            "UPDATE `artist`, (SELECT COUNT(DISTINCT `album_disk`.`id`) AS `album_disk_count`, `artist_map`.`artist_id` FROM `artist_map` LEFT JOIN `album` ON `album`.`id` = `artist_map`.`object_id` AND `artist_map`.`object_type` = 'album' LEFT JOIN `album_disk` ON `album_disk`.`album_id` = `album`.`id` LEFT JOIN `catalog` ON `catalog`.`id` = `album`.`catalog` WHERE `catalog`.`enabled` = '1' AND `album`.`enabled` = 1 GROUP BY `artist_map`.`artist_id`) AS `album_disk` SET `artist`.`album_disk_count` = `album_disk`.`album_disk_count` WHERE `artist`.`album_disk_count` != `album_disk`.`album_disk_count` AND `artist`.`id` = `album_disk`.`artist_id`;",
            "UPDATE `artist` SET `album_count` = 0, `album_disk_count` = 0 WHERE (`album_count` > 0 OR `album_disk_count` > 0) AND NOT EXISTS (SELECT 1 FROM `artist_map` INNER JOIN `album` ON `album`.`id` = `artist_map`.`object_id` WHERE `artist_map`.`artist_id` = `artist`.`id` AND `artist_map`.`object_type` = 'album' AND `album`.`enabled` = 1);",
            "UPDATE `artist`, (SELECT COUNT(`song`.`id`) AS `song_count`, `artist_map`.`artist_id` FROM `artist_map` LEFT JOIN `song` ON `song`.`id` = `artist_map`.`object_id` AND `artist_map`.`object_type` = 'song' LEFT JOIN `catalog` ON `catalog`.`id` = `song`.`catalog` WHERE `catalog`.`enabled` = '1' AND `song`.`enabled` = 1 GROUP BY `artist_map`.`artist_id`) AS `song` SET `artist`.`song_count` = `song`.`song_count` WHERE `artist`.`song_count` != `song`.`song_count` AND `artist`.`id` = `song`.`artist_id`;",

            // an artist whose songs were all disabled keeps its map rows, so the join above still misses it
            // and the count it kept would go on advertising songs nobody can reach
            "UPDATE `artist` SET `song_count` = 0 WHERE `song_count` > 0 AND NOT EXISTS (SELECT 1 FROM `artist_map` LEFT JOIN `song` ON `song`.`id` = `artist_map`.`object_id` WHERE `artist_map`.`object_type` = 'song' AND `artist_map`.`artist_id` = `artist`.`id` AND `song`.`enabled` = 1);",
    ...
    public function updateCounts(int $artistId): void
    {
        ...
        $statements = [
            ...
            ["UPDATE `artist`, (SELECT COUNT(DISTINCT `album`.`id`) AS `album_count`, `artist_map`.`artist_id` FROM `artist_map` LEFT JOIN `album` ON `album`.`id` = `artist_map`.`object_id` AND `artist_map`.`object_type` = 'album' LEFT JOIN `catalog` ON `catalog`.`id` = `album`.`catalog` WHERE `artist_map`.`artist_id` = ? AND `catalog`.`enabled` = '1' AND `album`.`enabled` = 1 GROUP BY `artist_map`.`artist_id`) AS `album` SET `artist`.`album_count` = `album`.`album_count` WHERE `artist`.`album_count` != `album`.`album_count` AND `artist`.`id` = `album`.`artist_id`;", [$artistId]],
            ["UPDATE `artist`, (SELECT COUNT(DISTINCT `album_disk`.`id`) AS `album_disk_count`, `artist_map`.`artist_id` FROM `artist_map` LEFT JOIN `album` ON `album`.`id` = `artist_map`.`object_id` AND `artist_map`.`object_type` = 'album' LEFT JOIN `album_disk` ON `album_disk`.`album_id` = `album`.`id` LEFT JOIN `catalog` ON `catalog`.`id` = `album`.`catalog` WHERE `artist_map`.`artist_id` = ? AND `catalog`.`enabled` = '1' AND `album`.`enabled` = 1 GROUP BY `artist_map`.`artist_id`) AS `album_disk` SET `artist`.`album_disk_count` = `album_disk`.`album_disk_count` WHERE `artist`.`album_disk_count` != `album_disk`.`album_disk_count` AND `artist`.`id` = `album_disk`.`artist_id`;", [$artistId]],
            ["UPDATE `artist` SET `album_count` = 0, `album_disk_count` = 0 WHERE `artist`.`id` = ? AND (`album_count` > 0 OR `album_disk_count` > 0) AND NOT EXISTS (SELECT 1 FROM `artist_map` INNER JOIN `album` ON `album`.`id` = `artist_map`.`object_id` WHERE `artist_map`.`artist_id` = `artist`.`id` AND `artist_map`.`object_type` = 'album' AND `album`.`enabled` = 1);", [$artistId]],
            ["UPDATE `artist`, (SELECT COUNT(`song`.`id`) AS `song_count`, `artist_map`.`artist_id` FROM `artist_map` LEFT JOIN `song` ON `song`.`id` = `artist_map`.`object_id` AND `artist_map`.`object_type` = 'song' LEFT JOIN `catalog` ON `catalog`.`id` = `song`.`catalog` WHERE `artist_map`.`artist_id` = ? AND `catalog`.`enabled` = '1' AND `song`.`enabled` = 1 GROUP BY `artist_map`.`artist_id`) AS `song` SET `artist`.`song_count` = `song`.`song_count` WHERE `artist`.`song_count` != `song`.`song_count` AND `artist`.`id` = `song`.`artist_id`;", [$artistId]],

            // an artist whose songs were all disabled keeps its map rows, so the join above still misses it
            // and the count it kept would go on advertising songs nobody can reach
            ["UPDATE `artist` SET `song_count` = 0 WHERE `id` = ? AND `song_count` > 0 AND NOT EXISTS (SELECT 1 FROM `artist_map` LEFT JOIN `song` ON `song`.`id` = `artist_map`.`object_id` WHERE `artist_map`.`object_type` = 'song' AND `artist_map`.`artist_id` = `artist`.`id` AND `song`.`enabled` = 1);", [$artistId]],
