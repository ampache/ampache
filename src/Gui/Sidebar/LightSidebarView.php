...
/**
 * The narrow icon-only sidebar.
 *
 * It offers the same destinations as the top menu, so the two lists are worth keeping in step.
 */
final class LightSidebarView extends AbstractSidebarView
{
    ...
    {
        return (AmpConfig::get('sidebar_light') && AmpConfig::get('sidebar_hide_switcher'))
    ...
    {
        $items = [
            ['url' => '/mashup.php?action=artist', 'icon' => 'topmenu-artist', 'label' => T_('Artists'), 'target' => false],
            ['url' => '/mashup.php?action=' . $this->albumType, 'icon' => 'topmenu-album', 'label' => T_('Albums'), 'target' => false],
        ];

        if (!AmpConfig::get('sidebar_hide_playlist')) {
            $items[] = ['url' => '/mashup.php?action=playlist', 'icon' => 'topmenu-playlist', 'label' => T_('Playlists'), 'target' => false];
        }
        ...
        $items[] = ['url' => '/browse.php?action=tag&type=' . $this->albumType, 'icon' => 'topmenu-tagcloud', 'label' => T_('Genres'), 'target' => false];
    ...
    {
        return $this->findTemplate('sidebar/light.phtml');
    }
}
