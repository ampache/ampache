<?php

declare(strict_types=1);

/**
 * vim:set softtabstop=4 shiftwidth=4 expandtab:
 *
 * LICENSE: GNU Affero General Public License, version 3 (AGPL-3.0-or-later)
 * Copyright Ampache.org, 2001-2026
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <https://www.gnu.org/licenses/>.
 *
 */

namespace Ampache\Gui\Label;

use Ampache\Gui\View\AbstractView;
use Ampache\Repository\Model\Label;
use Override;

/**
 * One row of the labels browse.
 */
final class LabelRowView extends AbstractView
{
    public function __construct(
        private readonly string $webPath,
        private readonly Label $label,
        private readonly string $coverCellClass,
        private readonly bool $mayInteract,
        private readonly bool $mayAddToPlaylist,
        private readonly bool $sociable,
        private readonly bool $mayDelete,
    ) {}

    public function getCoverCellClass(): string
    {
        return $this->coverCellClass;
    }

    public function getLabel(): Label
    {
        return $this->label;
    }

    public function getLabelUrl(): string
    {
        return $this->webPath . '/labels.php?action=show&label=' . $this->label->id;
    }

    public function getName(): string
    {
        return (string) $this->label->get_fullname();
    }

    public function getWebPath('/client'): string
    {
        return $this->webPath;
    }

    public function isSociable(): bool
    {
        return $this->sociable;
    }

    public function mayAddToPlaylist(): bool
    {
        return $this->mayAddToPlaylist;
    }

    public function mayDelete(): bool
    {
        return $this->mayDelete;
    }

    /**
     * An install with authentication off shows the actions to everyone; otherwise they need an account.
     */
    public function mayInteract(): bool
    {
        return $this->mayInteract;
    }

    #[Override]
    protected function templateFile(): string
    {
        return $this->findTemplate('label_row.phtml');
    }
}
