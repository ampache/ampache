...
class AmpacheDeezer extends AmpachePlugin implements PluginSongPreviewInterface
{
    ...
    #[Override]
    public string $description = 'Song preview from Deezer';
    ...
    #[Override]
    public string $url = 'https://www.deezer.com';
    ...
    {
        $this->description = T_('Song preview from Deezer');
    }
    ...
    public function get_song_preview(string $track_mbid, string $artist_name, string $title): array
    {
        ...
        $response = $this->_query_server('https://api.deezer.com/search?limit=5&q=' . rawurlencode($artist_name . ' ' . $title));
        if ($response === null) {
            return [];
        }
        ...
        foreach ($response->data ?? [] as $row) {
            $file = (string) ($row->preview ?? '');
            if ($file === '') {
                continue;
            }

            $results[] = new SongPreviewResult($file, (string) ($row->title ?? $title), (string) ($row->artist->name ?? $artist_name));
        }
