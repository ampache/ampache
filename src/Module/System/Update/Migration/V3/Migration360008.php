...
final class Migration360008 extends AbstractMigration
{
    ...
    public function migrate(): void
    {
        ...
        if (!$remote_username) {
            ...
            $this->updateDatabase("ALTER TABLE `catalog` ADD `remote_username` VARCHAR (255) AFTER `catalog_type`");
        }
        if (!$remote_password) {
            $this->updateDatabase("ALTER TABLE `catalog` ADD `remote_password` VARCHAR (255) AFTER `remote_username`");
        }
    }
}
