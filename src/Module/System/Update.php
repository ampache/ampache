...
class Update
{
    ...
    public static function update_530001()
    {
        ...
        $retval &= (Dba::write($sql) !== false);
        // fill the data
        $sql = "REPLACE INTO `album_map` (`album_id`, `object_type`, `object_id`) SELECT DISTINCT `artist_map`.`object_id` AS `album_id`, 'artist', `artist_map`.`artist_id` AS `object_id` FROM `artist_map` WHERE `artist_map`.`object_type` = 'album' UNION SELECT DISTINCT `song`.`album` AS `album_id`, 'artist', `song`.`artist` AS `object_id` FROM `song` WHERE `song`.`artist` > 0 UNION SELECT DISTINCT `song`.`album` AS `album_id`, 'artist', `artist_map`.`artist_id` AS `object_id` FROM `artist_map` LEFT JOIN `song` ON `artist_map`.`object_type` = 'song' AND `artist_map`.`object_id` = `song`.`id` WHERE `song`.`artist` > 0;";
        $retval &= (Dba::write($sql) !== false);
