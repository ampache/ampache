...
class AutoUpdate
{
    ...
    public static function github_request(string $action): ?object
    {
        try {
            ...
            debug_event(self::class, 'Request error: ' . $exception->getMessage(), 1);
    ...
    public static function get_latest_version(?bool $force = false): string
    {
        ...
        foreach ($tags as $release) {
            $str = strstr((string) $release->name, "-"); // ignore ALL tagged releases (e.g. 4.2.5-preview 4.2.5-beta)
            if (in_array($str, ['', '0', false], true)) {
    ...
    public static function show_new_version(): void
    {
        ...
        if (self::_is_git_repository()) {
            echo ' | <a class="nohtml" href="' . AmpConfig::get_web_path('/client') . '/update.php?type=sources&action=update"> <b>' . T_('Update') . '</b></a>';
            echo ' | <a class="nohtml" href="' . AmpConfig::get_web_path('/client') . '/update.php?type=sources&action=clear">' . T_('Ignore') . '</a>';
        }
