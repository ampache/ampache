...
class Catalog_local extends Catalog
{
    ...
    private function _verify_chunk($tableName, $chunk, $chunk_size)
    {
        ...
        $sql = ($tableName == 'album')
            ? "SELECT `song`.`album` AS `id`, MIN(`song`.`file`) AS `file`, MIN(`song`.`update_time`) AS `update_time` FROM `song` WHERE `song`.`album` IN (SELECT `song`.`album` FROM `song` LEFT JOIN `catalog` ON `song`.`catalog` = `catalog`.`id` WHERE `song`.`catalog` = " . $this->id . ") GROUP BY `song`.`album` ORDER BY `song`.`update_time` LIMIT $count, $chunk_size"
