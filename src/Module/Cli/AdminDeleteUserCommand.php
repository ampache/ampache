...
final class AdminDeleteUserCommand extends Command
{
    public function __construct(
        private readonly UserRepositoryInterface $userRepository,
    ) {
        parent::__construct('admin:deleteUser', T_('Delete a User'));

        $this
            ->option('-u|--user', T_('User ID'), 'intval', 0)
            ->argument('[username]', T_('Username'))
            ->usage('<bold>  admin:deleteUser some-user</end> <comment> ## ' . T_('Delete the User with the name `some-user`') . '</end><eol/>');
    }
