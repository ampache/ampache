...
final class AdminEnableUserCommand extends Command
{
    public function __construct(
        private readonly UserRepositoryInterface $userRepository,
        private readonly UserStateTogglerInterface $userStateToggler,
    ) {
        parent::__construct('admin:enableUser', T_('Enable a User'));

        $this
            ->option('-u|--user', T_('User ID'), 'intval', 0)
            ->argument('[username]', T_('Username'))
            ->usage('<bold>  admin:enableUser some-user</end> <comment> ## ' . T_('Enable the User with the name `some-user`') . '</end><eol/>');
    }
    ...
    public function execute(
        ...
        $interactor->ok(
            sprintf(T_('%s has been enabled'), $user->getUsername()),
            true
        );
    }
