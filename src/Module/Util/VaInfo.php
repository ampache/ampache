...
class VaInfo
{
    ...
    public static function clean_tag_info($results, $keys, $filename = null)
    {
        ...
        foreach ($keys as $key) {
            ...
            $info['lyrics']                = $info['lyrics'] ?: strip_tags(nl2br((string)$tags['lyrics']), "<br>");
            ...
            foreach ($this->_raw['tags'] as $key => $tag_array) {
                switch ($key) {
                    ...
                    case 'lyrics3':
                        ...
                        break;
                    default:
                        debug_event('vainfo.class',
                            'Cleaning unrecognised tag type ' . $key . ' for file ' . $this->filename, 5);
                        $parsed = $this->_cleanup_generic($tag_array);
                        break;
                }
    ...
    private function _clean_type($type)
    {
        switch ($type) {
            ...
            case 'quicktime':
                return $type;
            default:
                /* Log the fact that we couldn't figure it out */ debug_event('vainfo.class',
                'Unable to determine file type from ' . $type . ' on file ' . $this->filename, 3);
                        ...
                        if ($matches != null) {
                            ...
                            if (isset($matches[5])) {
                                ...
                                if (preg_match("~^(\d\d)[\_\-\.\s]?(.*)~", $file, $matches)) {
                                    $results['Ampache\Model\TVShow_Episode'] = $matches[1];
                                    ...
                                    $results['Ampache\Model\TVShow_Episode'] = $matches[2];
                                }
                            }
                        }
                    }
