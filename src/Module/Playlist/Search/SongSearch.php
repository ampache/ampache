            ...
            switch ($rule[0]) {
                ...
                case 'last_play':
                    ...
                    $table['last_play'] .= (!strpos((string) $table['last_play'], "last_play_" . $my_type . "_" . $user_id))
                        ...
                        : "";
                    $where[] = "`last_play_" . $my_type . "_" . $user_id . "`.`date` $operator_sql (UNIX_TIMESTAMP() - ($input * 86400))";
                    break;
                case 'last_skip':
                    $my_type = 'song';
                    ...
                    $table['last_skip'] .= (!strpos((string) $table['last_skip'], "last_skip_" . $my_type . "_" . $user_id))
                        ...
                        : "";
                    $where[] = "`last_skip_" . $my_type . "_" . $user_id . "`.`date` $operator_sql (UNIX_TIMESTAMP() - ($input * 86400))";
                    break;
                case 'last_play_or_skip':
                    ...
                    $table['last_play_or_skip'] .= (!strpos((string) $table['last_play_or_skip'], "last_play_or_skip_" . $my_type . "_" . $user_id))
                        ...
                        : "";
                    $where[] = "`last_play_or_skip_" . $my_type . "_" . $user_id . "`.`date` $operator_sql (UNIX_TIMESTAMP() - ($input * 86400))";
                    break;
                case 'played_times':
                ...
                case 'rating':
                    ...
                    break;
                case 'favorite':
                    $where[]      = "`song`.`title` $operator_sql ? AND `favorite_song_$user_id`.`user` = $user_id AND `favorite_song_$user_id`.`object_type` = 'song'";
                    $parameters[] = $input;
                    // flag once per user
                ...
                case 'other_user_artist':
                    ...
                    if ($operator_sql == 'userflag') {
                        ...
                        $where[] = ($unrated) ? "`song`.`$column` NOT IN (SELECT `object_id` FROM `rating` WHERE `object_type` = '$my_type' AND `user` = $other_userid)" : "`rating_" . $my_type . "_" . $other_userid . "`.$operator_sql AND `rating_" . $my_type . "_" . $other_userid . "`.`user` = $other_userid AND `rating_" . $my_type . "_" . $other_userid . "`.`object_type` = '$my_type'";
                        // rating once per user
                        if (!array_key_exists('rating', $table)) {
                            $table['rating'] = '';
                        }
