        ...
        if ($licenseId > 0) {
            ...
            if ($license->id) {
                $this->licenseRepository->update(
                    $licenseId,
                    (array_key_exists('name', $data)) ? filter_var($data['name'], FILTER_SANITIZE_FULL_SPECIAL_CHARS) : '',
                    (array_key_exists('description', $data)) ? filter_var($data['description'], FILTER_SANITIZE_FULL_SPECIAL_CHARS)  : '',
