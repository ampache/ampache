...
class WebPlayer
{
    ...
    public static function can_transcode(
        ...
        if (($transcode_cfg === 'always' || $force_type !== '' && $force_type !== '0' || !in_array('native', $valid_types) || $types['real'] != $file_type) && ($transcode_cfg === 'always' || $transcode_cfg !== 'never' && in_array('transcode', $valid_types))) {
            ...
            if (!$transcode && ($transcode_cfg === 'always' || !in_array('native', $valid_types))) {
                $transcode_settings = Stream::get_transcode_settings_for_media($file_type, $force_type, 'webplayer', $media_type);
                if ($transcode_settings !== []) {
                    $transcode = true;
                }
    ...
    public static function get_media_js_param(
        ...
        foreach (['title', 'author'] as $member) {
            $kmember = $member === "author" ? "artist" : $member;
        ...
        if ($media != null) {
            ...
            if ($media instanceof Song_Preview) {
                // a preview is always the provider's own url, reached by a 303 from play/index.php
                $json['remote'] = true;
            } elseif ($media instanceof Song || $media instanceof Video || $media instanceof Podcast_Episode) {
