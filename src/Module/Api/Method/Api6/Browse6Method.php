...
final class Browse6Method
{
    ...
    public static function browse(array $input, User $user): bool
    {
        ...
        if ($object_type === 'root') {
            ...
            if ($object_id === null && !Api6::check_parameter($input, ['filter'], self::ACTION)) {
                return false;
            }
            ...
            if (!Api6::check_parameter($input, ['filter', 'catalog'], self::ACTION)) {
                return false;
            }
        ...
        switch ($input['api_format']) {
            case 'json':
                ...
                echo Json6_Data::browses($results, $object_type, $child_type, $object_id, $catalog_id);
                break;
            default:
                Xml6_Data::set_offset((int) ($input['offset'] ?? 0));
                Xml6_Data::set_limit($input['limit'] ?? 0);
                echo Xml6_Data::browses($results, $object_type, $child_type, $object_id, $catalog_id);
        }
