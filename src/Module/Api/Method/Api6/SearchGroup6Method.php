                ...
                foreach ($results as $key => $search) {
                    ...
                    switch ($key) {
                        case 'album':
                            ...
                            $output['search'][$key] = Json6_Data::albums_array($search, [], $user, $input['auth'], false);
                            break;
                        case 'song_artist':
                        case 'album_artist':
                        case 'artist':
                            ...
                            $output['search'][$key] = Json6_Data::artists_array($search, [], $user, $input['auth'], false);
                            break;
                        case 'label':
                            $output['search'][$key] = Json6_Data::labels_array($search);
                            break;
                        case 'playlist':
                            $output['search'][$key] = Json6_Data::playlists_array($search, $user, $input['auth']);
                            break;
                        case 'podcast':
                            $output['search'][$key] = Json6_Data::podcasts_array($search, $user, $input['auth']);
                            break;
                        case 'podcast_episode':
                            if ((count($search) > $limit || $offset > 0) && $limit) {
                                $search = array_slice($search, $offset, $limit);
                            }
                            $output['search'][$key] = Json6_Data::podcast_episodes_array($search, $user, $input['auth'], false);
                            break;
                        case 'genre':
                        case 'tag':
                            $output['search'][$key] = Json6_Data::genres_array($search);
                            break;
                        case 'user':
                            $output['search'][$key] = Json6_Data::users_array($search);
                            break;
                        case 'video':
                            $output['search'][$key] = Json6_Data::videos_array($search, $user, $input['auth']);
                            break;
                        case 'song':
                            if ((count($search) > $limit || $offset > 0) && $limit) {
                                $search = array_slice($search, $offset, $limit);
                            }
                            $output['search'][$key] = Json6_Data::songs_array($search, $user, $input['auth']);
                            break;
                    }
                }
