...
/**
 * Class PlaylistRemoveSong6Method
 * @package Lib\Api6Methods
 */
final class PlaylistRemoveSong6Method
{
    ...
    public static function playlist_remove_song(array $input, User $user): bool
    {
        ...
        if (array_key_exists('clear', $input) && (int)$input['clear'] === 1) {
            ...
            Api6::message('song removed from playlist', $input['api_format']);
        } elseif (array_key_exists('track', $input)) {
            $track = (int) scrub_in((string) $input['track']);
            if (!$playlist->has_item(null, $track)) {
                Api6::error('Not Found', ErrorCodeEnum::NOT_FOUND, self::ACTION, 'track', $input['api_format']);
