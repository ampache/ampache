...
final class PlaylistGenerate6Method
{
    ...
    public static function playlist_generate(array $input, User $user): bool
    {
        ...
        if (empty($results)) {
            Api6::empty($format, $input['api_format']);
            ...
            case 'id':
                switch ($input['api_format']) {
                    case 'json':
                        ...
                        break;
                    default:
                        echo Xml6_Data::keyed_array($results, false, 'id');
                }
                break;
            case 'index':
                switch ($input['api_format']) {
                    case 'json':
                        echo Json6_Data::indexes($results, 'song', $user, $input['auth']);
                        break;
                    default:
                        echo Xml6_Data::indexes($results, 'song', $user, $input['auth']);
                }
                break;
            case 'song':
            default:
