...
final class DeletedPodcastEpisodes6Method
{
    ...
    public static function deleted_podcast_episodes(array $input, User $user): bool
    {
        ...
        if ($items === []) {
            Api6::empty('deleted_podcast_episodes', $input['api_format']);
        ...
        switch ($input['api_format']) {
            case 'json':
                ...
                Json6_Data::set_limit((int) ($input['limit'] ?? 0));
                echo Json6_Data::deleted('podcast_episode', $items);
                break;
            default:
                Xml6_Data::set_offset((int) ($input['offset'] ?? 0));
                Xml6_Data::set_limit((int) ($input['limit'] ?? 0));
                echo Xml6_Data::deleted('podcast_episode', $items);
        }
