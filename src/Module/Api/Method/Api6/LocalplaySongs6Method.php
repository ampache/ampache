...
/**
 * Class LocalplaySongs6Method
 * @package Lib\Api6Methods
 */
final class LocalplaySongs6Method
{
    ...
    public static function localplay_songs(array $input, User $user): bool
    {
        ...
        if (!Api6::check_access(AccessTypeEnum::LOCALPLAY, $level, $user->id, self::ACTION, $input['api_format'])) {
            return false;
        }
        ...
        $songs = $localplay->get();
        if (empty($songs)) {
            Api6::empty('localplay_songs', $input['api_format']);
        ...
        switch ($input['api_format']) {
            case 'json':
                ...
                break;
            default:
                echo Xml6_Data::object_array($results['localplay_songs'], 'localplay_songs');
        }
