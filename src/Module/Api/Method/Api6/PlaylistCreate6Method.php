...
/**
 * Class PlaylistCreate6Method
 * @package Lib\Api6Methods
 */
final class PlaylistCreate6Method
{
    ...
    {
        if (!Api6::check_parameter($input, ['name'], self::ACTION)) {
            return false;
        }
        $name = $input['name'];
        ...
        switch ($input['api_format']) {
            case 'json':
                echo Json6_Data::playlists([$object_id], $user, $input['auth'], false, false);
                break;
            default:
                echo Xml6_Data::playlists([$object_id], $user, $input['auth']);
        }
