...
final class PlaylistGenerate5Method
{
    ...
    public static function playlist_generate(array $input, User $user): bool
    {
        ...
        if (!in_array($format, ['song', 'index', 'id'])) {
            Api5::error(ErrorCodeEnum::BAD_REQUEST, sprintf(T_('Bad Request: %s'), $format), self::ACTION, 'type', $input['api_format']);
            ...
            case 'id':
                switch ($input['api_format']) {
                    case 'json':
                        ...
                        break;
                    default:
                        echo Xml5_Data::keyed_array($results, false, 'id');
                }
                break;
            case 'index':
                switch ($input['api_format']) {
                    case 'json':
                        echo Json5_Data::indexes($results, 'song', $user, $input['auth']);
                        break;
                    default:
                        echo Xml5_Data::indexes($results, 'song', $user, $input['auth']);
                }
                break;
            case 'song':
            default:
