...
final class PlaylistGenerate5Method
{
    ...
    public static function playlist_generate(array $input): bool
    {
        ...
        if (!in_array($format, array('song', 'index', 'id'))) {
            Api5::error(sprintf(T_('Bad Request: %s'), $format), '4710', self::ACTION, 'type', $input['api_format']);
            ...
            case 'id':
                switch ($input['api_format']) {
                    case 'json':
                        ...
                        break;
                    default:
                        echo Xml5_Data::keyed_array($song_ids, false, 'id');
                }
                break;
            case 'index':
                switch ($input['api_format']) {
                    case 'json':
                        echo Json5_Data::indexes($song_ids, 'song', $user->id);
                        break;
                    default:
                        echo Xml5_Data::indexes($song_ids, 'song', $user->id);
                }
                break;
            case 'song':
            default:
                switch ($input['api_format']) {
                    case 'json':
                        echo Json5_Data::songs($song_ids, $user->id);
                        break;
                    default:
                        echo Xml5_Data::songs($song_ids, $user->id);
                }
        }
