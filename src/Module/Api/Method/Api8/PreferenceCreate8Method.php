...
/**
 * Class PreferenceCreate8Method
 * @package Lib\Api8Methods
 */
final class PreferenceCreate8Method
{
    ...
    public static function preference_create(array $input, User $user): bool
    {
        ...
        if (!in_array($category, ['interface', 'internal', 'options', 'playlist', 'plugins', 'streaming'])) {
            Api::error(ErrorCodeEnum::BAD_REQUEST, sprintf('Bad Request: %s', $type), self::ACTION, 'category', $input['api_format']);
        ...
        if (empty($results)) {
            /* HINT: Requested object string/id/type ("album", "myusername", "some song title", 1298376) */
            Api::error(ErrorCodeEnum::NOT_FOUND, sprintf('Not Found: %s', $pref_name), self::ACTION, 'system', $input['api_format']);
