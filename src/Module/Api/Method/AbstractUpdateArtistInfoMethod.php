 ...
 */
abstract class AbstractUpdateArtistInfoMethod implements MethodInterface
{
    ...
    public function handle(
        ...
        $objectId = (int) $filter;
        $item     = $this->modelFactory->createArtist($objectId);
        if ($item->isNew()) {
        ...
        $info = Recommendation::get_artist_info($objectId);
        $like = Recommendation::get_artists_like($objectId);
        ...
        $response->getBody()->write(
            $output->success($apiVersion, 'Updated artist info: ' . $objectId)
        );
