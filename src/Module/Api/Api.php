...
class Api
{
    ...
    public static function handshake($input)
    {
        ...
        // Log this attempt
        debug_event('api.class',
            "Login Attempt, IP:$user_ip Time: $timestamp User:$username ($user_id) Auth:$passphrase", 1);
        ...
        if ($user_id > 0 && Access::check_network('api', $user_id, 5)) {
            ...
            if ($username) {
                ...
                if (($timestamp < (time() - 1800)) || ($timestamp > (time() + 1800))) {
                    ...
                    self::message('error',
                        T_('Received Invalid Handshake') . ' - ' . T_('Login failed, timestamp is out of range'), '401',
                        $input['api_format']);
        ...
        self::message('error', T_('Received Invalid Handshake') . ' - ' . T_('Incorrect username or password'), '401',
            $input['api_format']);
    ...
    public static function ping($input)
    {
        ...
        if (Session::exists('api', $input['auth'])) {
            ...
            $xmldata = array_merge(array(
                'session_expire' => date("c", time() + (int)AmpConfig::get('session_length') - 60)
            ), $xmldata);
        }
    ...
    public static function goodbye($input)
    {
        ...
        if (Session::exists('api', $input['auth'])) {
            ...
            debug_event('api.class',
                'Goodbye Received from ' . Core::get_server('REMOTE_ADDR') . ' :: ' . $input['auth'], 5);
            ob_end_clean();
    ...
    public static function get_indexes($input)
    {
        ...
        if (!in_array($type, array('song', 'album', 'artist', 'playlist'))) {
            self::message('error', T_('Incorrect object type') . ' ' . $type, '400', $input['api_format']);

            return false;
        }
        self::getBrowse()->reset_filters();
        self::getBrowse()->set_type($type);
        self::getBrowse()->set_sort('name', 'ASC');
        ...
        if ($type == 'playlist') {
            self::getBrowse()->set_filter('playlist_type', $user->id);
            $objects = array_merge(self::getBrowse()->get_objects(), Playlist::get_smartlists(true, $user->id));
        } else {
            $objects = self::getBrowse()->get_objects();
        }
    ...
    {
        self::getBrowse()->reset_filters();
        self::getBrowse()->set_type('artist');
        self::getBrowse()->set_sort('name', 'ASC');
    ...
    public static function licenses($input)
    {
        if (!AmpConfig::get('licensing')) {
            self::message('error', T_('Access Denied: licensing features are not enabled.'), '403', $input['api_format']);

            return false;
        }

        self::getBrowse()->reset_filters();
        self::getBrowse()->set_type('license');
        self::getBrowse()->set_sort('name', 'ASC');

        $method = $input['exact'] ? 'exact_match' : 'alpha_match';
        self::set_filter($method, $input['filter']);
        $licenses = self::getBrowse()->get_objects();
    ...
    public static function genres($input)
    {
        ...
        $method = $input['exact'] ? 'exact_match' : 'alpha_match';
        self::set_filter($method, $input['filter']);
        $tags = self::getBrowse()->get_objects();
    ...
    {
        self::getBrowse()->reset_filters();
        self::getBrowse()->set_type('song');
        self::getBrowse()->set_sort('title', 'ASC');
        ...
        $songs = self::getBrowse()->get_objects();
        $user  = User::get_from_username(Session::username($input['auth']));
    ...
    public static function playlists($input)
    {
        ...
        $playlist_ids = array_merge($playlist_ids,
            Playlist::get_smartlists($public, $userid, (string)$input['filter'], $method));
    ...
    public static function playlist_edit($input)
    {
        ...
        if (!($name || $type) && !$change_made) {
            self::message('error', T_('Nothing was changed'), '400', $input['api_format']);
    ...
    public static function playlist_add_song($input)
    {
        ...
        if ((AmpConfig::get('unique_playlist') || (int)$input['check'] == 1) && in_array($song,
                $playlist->get_songs())) {
    ...
    {
        // parameter defaults
        $mode = (!in_array($input['mode'], array('forgotten', 'recent', 'unplayed', 'random'),
            true)) ? 'random' : $input['mode'];
    ...
    public static function shares($input)
    {
        if (!AmpConfig::get('share')) {
            self::message('error', T_('Access Denied: sharing features are not enabled.'), '403', $input['api_format']);
        ...
        $shares = self::getBrowse()->get_objects();
    ...
    public static function share_create($input)
    {
        ...
        if (!in_array($object_type, array('song', 'album', 'artist'))) {
            self::message('error', T_('Wrong object type ' . $object_type), '400', $input['api_format']);

            return false;
        }
        $share = array();
        if (!InterfaceImplementationChecker::is_library_item($object_type) || !$object_id) {
            self::message('error', T_('Wrong library item type'), '400', $input['api_format']);
        } else {
    ...
    public static function share_delete($input)
    {
        ...
        if (in_array($object_id, Share::get_share_list())) {
            if (Share::delete_share($object_id, $user)) {
                ...
                self::message('error', 'share ' . $object_id . ' was not deleted', '400', $input['api_format']);
            }
        } else {
    ...
    public static function share_edit($input)
    {
        ...
        if (in_array($share_id, Share::get_share_list())) {
            ...
            if ($share->update($data, $user)) {
                ...
                self::message('error', 'share ' . $share_id . ' was not updated', '400', $input['api_format']);
            }
        } else {
    ...
    {
        self::getBrowse()->reset_filters();
        self::getBrowse()->set_type('video');
        self::getBrowse()->set_sort('title', 'ASC');

        $method = $input['exact'] ? 'exact_match' : 'alpha_match';
        Api::set_filter($method, $input['filter']);

        $video_ids = self::getBrowse()->get_objects();
        $user      = User::get_from_username(Session::username($input['auth']));
    ...
    public static function podcasts($input)
    {
        if (!AmpConfig::get('podcast')) {
            self::message('error', T_('Access Denied: podcast features are not enabled.'), '403', $input['api_format']);

            return false;
        }
        self::getBrowse()->reset_filters();
        self::getBrowse()->set_type('podcast');
        self::getBrowse()->set_sort('title', 'ASC');
        ...
        $podcasts = self::getBrowse()->get_objects();
    ...
    public static function podcast_create($input)
    {
        ...
        if ($podcast) {
            ...
            self::message('error', T_('Failed: podcast was not created.'), '400', $input['api_format']);
        }
        Session::extend($input['auth']);
    ...
    public static function podcast_delete($input)
    {
        ...
        if ($podcast->id) {
            if ($podcast->remove()) {
                ...
                self::message('error', 'podcast ' . $object_id . ' was not deleted', '400', $input['api_format']);
            }
        } else {
            self::message('error', 'podcast ' . $object_id . ' was not found', '404', $input['api_format']);
        }
    ...
    public static function podcast_edit($input)
    {
        ...
        if ($podcast->id) {
            ...
            if ($podcast->update($data)) {
                ...
                self::message('error', 'podcast ' . $podcast_id . ' was not updated', '400', $input['api_format']);
            }
        } else {
    ...
    public static function podcast_episode_delete($input)
    {
        ...
        if ($episode->id > 0) {
            if ($episode->remove()) {
                ...
                self::message('error', 'podcast_episode ' . $object_id . ' was not deleted', '400', $input['api_format']);
            }
        } else {
            self::message('error', 'podcast_episode ' . $object_id . ' was not found', '404', $input['api_format']);
        }
    ...
    {
        if (!self::check_access('interface', 100, User::get_from_username(Session::username($input['auth']))->id,
            'user_create', $input['api_format'])) {
            return false;
        }
    ...
    public static function user_update($input)
    {
        ...
        if ($password && Access::check('interface', 100, $user_id)) {
            self::message('error', 'Do not update passwords for admin users! ' . $username, '400',
                $input['api_format']);
    ...
    public static function user_delete($input)
    {
        ...
        if ($user->id && Session::username($input['auth']) != $username && !Access::check('interface', 100,
                $user->id)) {
            $user->delete();
    ...
    public static function followers($input)
    {
        if (!AmpConfig::get('sociable')) {
            self::message('error', T_('Access Denied: social features are not enabled.'), '403', $input['api_format']);
        ...
        if (!empty($username)) {
            ...
            if ($user !== null) {
                ...
                if (!count($users)) {
                    self::message('error', 'User `' . $username . '` has no followers.', '404', $input['api_format']);
                } else {
                    ob_end_clean();
                    switch ($input['api_format']) {
                        case 'json':
                ...
                debug_event('api.class', 'User `' . $username . '` cannot be found.', 1);
                self::message('error', 'User `' . $username . '` cannot be found.', '404', $input['api_format']);
            }
        }
        Session::extend($input['auth']);
    ...
    public static function following($input)
    {
        ...
        if (!empty($username)) {
            ...
            if ($user !== null) {
                ...
                if (!count($users)) {
                    self::message('error', 'User `' . $username . '` does not follow anyone.', '404', $input['api_format']);
                } else {
    ...
    public static function rate($input)
    {
        if (!AmpConfig::get('ratings')) {
            self::message('error', T_('Access Denied: Rating features are not enabled.'), '403', $input['api_format']);
        ...
        if (!in_array($rating, array('0', '1', '2', '3', '4', '5'))) {
            self::message('error', T_('Ratings must be between [0-5]. ' . $rating . ' is invalid'), '400', $input['api_format']);
    ...
    public static function flag($input)
    {
        if (!AmpConfig::get('userflags')) {
            self::message('error', T_('Access Denied: UserFlag features are not enabled.'), '403', $input['api_format']);
    ...
    public static function record_play($input)
    {
        ...
        debug_event('api.class',
            'record_play: ' . $item->id . ' for ' . $user->username . ' using ' . $agent . ' ' . (string)time(), 5);
    ...
    public static function scrobble($input)
    {
        ...
        debug_event('api.class', 'scrobble searching for:' . $song_name . ' - ' . $artist_name . ' - ' . $album_name,
            4);
        if (!$song_name || !$album_name || !$artist_name) {
            self::message('error', T_('Invalid input options'), '400', $input['api_format']);
        ...
        if ($scrobble_id === '') {
            ...
            debug_event('api.class',
                'scrobble: ' . $item->id . ' for ' . $user->username . ' using ' . $agent . ' ' . (string)time(), 5);
    ...
    public static function catalogs($input)
    {
        ...
        $filter = (in_array($input['filter'], array(
            'music',
            'clip',
            'tvshow',
            'movie',
    ...
    public static function catalog_action($input)
    {
        ...
        if (!self::check_access('interface', 75, User::get_from_username(Session::username($input['auth']))->id,
            'catalog_action', $input['api_format'])) {
            return false;
        }
        $task = (string)$input['task'];
        // confirm the correct data
        if (!in_array($task, array('add_to_catalog', 'clean_catalog', 'verify_catalog', 'gather_art'))) {
            self::message('error', T_('Incorrect catalog task') . ' ' . $task, '400', $input['api_format']);
    ...
    public static function catalog_file($input)
    {
        ...
        if (!AmpConfig::get('delete_from_disk') && $task == 'remove') {
            self::message('error', T_('Access Denied: delete from disk is not enabled.'), '403', $input['api_format']);

            return false;
        }
        if (!self::check_access('interface', 50, User::get_from_username(Session::username($input['auth']))->id,
            'catalog_file', $input['api_format'])) {
            return false;
        }
        ...
        if (!in_array($task, array('add', 'clean', 'verify', 'remove'))) {
            self::message('error', T_('Incorrect file task') . ' ' . $task, '400', $input['api_format']);
    ...
    public static function update_from_tags($input)
    {
        ...
        self::message('success', 'Updated tags for: ' . (string)$object . ' (' . $type . ')', null,
            $input['api_format']);
        Session::extend($input['auth']);
    ...
    public static function update_artist_info($input)
    {
        ...
        if (!self::check_access('interface', 75, User::get_from_username(Session::username($input['auth']))->id,
            'update_artist_info', $input['api_format'])) {
            return false;
        }
        $object = (int)$input['id'];
        ...
        self::message('error', T_('Failed to update_artist_info or recommendations for ' . (string)$object), '400',
            $input['api_format']);
        Session::extend($input['auth']);
    ...
    public static function update_podcast($input)
    {
        ...
        if ($podcast->id > 0) {
            if ($podcast->sync_episodes(true)) {
                self::message('success', 'Synced episodes for podcast: ' . (string)$object_id, null,
                    $input['api_format']);
                Session::extend($input['auth']);
            } else {
                self::message('error', T_('Failed to sync episodes for podcast: ' . (string)$object_id), '400',
                    $input['api_format']);
            }
        } else {
            self::message('error', 'podcast ' . $object_id . ' was not found', '404', $input['api_format']);
    ...
    public static function localplay($input)
    {
        ...
        switch ($input['command']) {
            ...
            default:
                ...
                self::message('error', T_('Invalid request'), '400', $input['api_format']);
    ...
    public static function democratic($input)
    {
        ...
        switch ($input['method']) {
            case 'vote':
                ...
                if (!$media->id) {
                    self::message('error', T_('Media object invalid or not specified'), '404', $input['api_format']);
                    break;
                }
