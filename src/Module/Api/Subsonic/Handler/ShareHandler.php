...
final class ShareHandler implements ShareHandlerInterface
{
    ...
    public function createshare(array $input, User $user): void
    {
        ...
        if (is_array($sub_id)) {
            $object      = Subsonic_Api::getAmpacheObject($sub_id[0]);
            $object_type = Subsonic_Api::getAmpacheType($sub_id[0]);
        } else {
        ...
        if (AmpConfig::get('share')) {
            ...
            if (is_array($sub_id) && $object_type === 'song') {
                ...
                $song_id     = Subsonic_Api::getAmpacheId($sub_id[0]);
                $tmp_song    = new Song($song_id);
                $sub_id      = Subsonic_Api::getAlbumSubId($tmp_song->album);
    ...
    public function deleteshare(array $input, User $user): void
    {
        ...
        if (AmpConfig::get('share')) {
            ...
            $share_id = Subsonic_Api::getAmpacheId($sub_id);
            $share    = ($share_id)
    ...
    public function updateshare(array $input, User $user): void
    {
        ...
        if (AmpConfig::get('share')) {
            $share = new Share(Subsonic_Api::getAmpacheId($sub_id));
            if ($share->id > 0) {
