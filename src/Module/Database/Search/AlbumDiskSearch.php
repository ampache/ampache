...
final class AlbumDiskSearch implements SearchInterface
{
    ...
    public function getSql(
        ...
        foreach ($search->rules as $rule) {
            ...
            switch ($rule[0]) {
                ...
                case 'my_flagged_artist':
                    ...
                    $column       = 'id';
                    $my_type      = ($looking == 'album') ? 'album_disk' : $looking;
                ...
                case 'played_times':
                    ...
                    break;
                case 'skipped_times':
                    $where[]      = sprintf('(`album_disk`.`total_skip` %s ?)', $operator_sql);
                    $parameters[] = $input;
                    break;
                case 'played_or_skipped_times':
                    $where[]      = sprintf('((`album_disk`.`total_count` + `album_disk`.`total_skip`) %s ?)', $operator_sql);
                    $parameters[] = $input;
                    break;
                case 'myplayed_times':
                    $my_type = 'album';
                ...
                case 'myplayed_or_skipped_times':
                    ...
                    break;
                case 'disk_count':
                    $where[]      = sprintf('`album_disk`.`disk_count` %s ?', $operator_sql);
                    $parameters[] = $input;
                    break;
                case 'song_count':
                    $where[]      = sprintf('`album_disk`.`song_count` %s ?', $operator_sql);
                    $parameters[] = $input;
                    break;
                case 'other_user':
                ...
                case 'duplicate_tracks':
                    $where[] = "`album_disk`.`id` IN (SELECT `album_disk`.`id` FROM `song` LEFT JOIN `album_disk` ON `album_disk`.`album_id` = `song`.`album` AND `album_disk`.`disk` = `song`.`disk` GROUP BY `album_disk`.`id`, `song`.`track` HAVING COUNT(`song`.`track`) > 1)";
                    break;
                case 'smartplaylist':
        ...
        if ($search->catalog_id) {
            if ($where_sql !== '' && $where_sql !== '0') {
                $where_sql = "(" . $where_sql . ") AND `album_disk`.`catalog` = " . $search->catalog_id;
            } else {
                $where_sql = "`album_disk`.`catalog` = " . $search->catalog_id;
            }
        }
