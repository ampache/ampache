...
final class SongSearch implements SearchInterface
{
    ...
    public function getSql(
            ...
            switch ($rule[0]) {
                ...
                case 'anywhere':
                    ...
                    if (in_array($operator_sql, ['!=', 'NOT LIKE', 'NOT'])) {
                        ...
                        $where[] = sprintf("((`artist`.`name` %s ? OR LTRIM(CONCAT(COALESCE(`artist`.`prefix`, ''), ' ', `artist`.`name`)) %s ?) OR (`album`.`name` %s ? OR LTRIM(CONCAT(COALESCE(`album`.`prefix`, ''), ' ', `album`.`name`)) %s ?) OR `song_data`.`comment` %s ? OR `song_data`.`label` %s ? OR `song`.`file` %s ? OR `song`.`title` %s ? OR ", $operator_sql, $operator_sql, $operator_sql, $operator_sql, $operator_sql, $operator_sql, $operator_sql, $operator_sql) . $tag_string . ')';
                    }
                ...
                case 'album_artist':
                    ...
                    break;
                case 'id':
                    $where[]      = sprintf('`song`.`id` %s ?', $operator_sql);
                    $parameters[] = $input;
                    break;
                case 'time':
                    $input        = ((int) $input) * 60;
                ...
                case 'composer':
                    if ($operator_sql === 'NOT SOUNDS LIKE') {
                        ...
                        $where[] = sprintf('`song`.`composer` %s ?', $operator_sql);
                    }
                ...
                case 'catalog':
                case 'license':
                    $where[]      = sprintf('`song`.`%s` %s ?', $rule[0], $operator_sql);
                    $parameters[] = $input;
                    break;
                case 'no_license':
                ...
                case 'comment':
                    ...
                    if ($operator_sql === 'NOT SOUNDS LIKE') {
                        ...
                        $where[] = sprintf('`song_data`.`comment` %s ?', $operator_sql);
                    }
                ...
                case 'label':
                    ...
                    if ($operator_sql === 'NOT SOUNDS LIKE') {
                        ...
                        $where[] = sprintf('`song_data`.`label` %s ?', $operator_sql);
                    }
                ...
                case 'lyrics':
                    ...
                    if ($operator_sql === 'NOT SOUNDS LIKE') {
                        ...
                        $where[] = sprintf('`song_data`.`lyrics` %s ?', $operator_sql);
                    }
                ...
                case 'last_play':
                    ...
                    $table['last_play'] .= (strpos($table['last_play'], "last_play_" . $my_type . "_" . $search_user_id))
                        ? ""
                        : "LEFT JOIN " . PlayHistorySubquery::lastDate($my_type, ['stream'], $search_user_id) . " AS `last_play_" . $my_type . "_" . $search_user_id . "` ON `song`.`id` = `last_play_" . $my_type . "_" . $search_user_id . "`.`object_id` AND `last_play_" . $my_type . "_" . $search_user_id . "`.`object_type` = '" . $my_type . "'";
                ...
                case 'last_skip':
                    ...
                    $table['last_skip'] .= (strpos($table['last_skip'], "last_skip_" . $my_type . "_" . $search_user_id))
                        ? ""
                        : "LEFT JOIN " . PlayHistorySubquery::lastDate($my_type, ['skip'], $search_user_id) . " AS `last_skip_" . $my_type . "_" . $search_user_id . "` ON `song`.`id` = `last_skip_" . $my_type . "_" . $search_user_id . "`.`object_id` AND `last_skip_" . $my_type . "_" . $search_user_id . "`.`object_type` = '" . $my_type . "' ";
                ...
                case 'last_play_or_skip':
                    ...
                    $table['last_play_or_skip'] .= (strpos($table['last_play_or_skip'], "last_play_or_skip_" . $my_type . "_" . $search_user_id))
                        ? ""
                        : "LEFT JOIN " . PlayHistorySubquery::lastDate($my_type, ['stream', 'skip'], $search_user_id) . " AS `last_play_or_skip_" . $my_type . "_" . $search_user_id . "` ON `song`.`id` = `last_play_or_skip_" . $my_type . "_" . $search_user_id . "`.`object_id` AND `last_play_or_skip_" . $my_type . "_" . $search_user_id . "`.`object_type` = '" . $my_type . "'";
                ...
                case 'played_times':
                    ...
                    break;
                case 'skipped_times':
                    $where[]      = sprintf('(`song`.`total_skip` %s ?)', $operator_sql);
                    $parameters[] = $input;
                    break;
                case 'played_or_skipped_times':
                    $where[]      = sprintf('((`song`.`total_count` + `song`.`total_skip`) %s ?)', $operator_sql);
                    $parameters[] = $input;
                    break;
                case 'play_skip_ratio':
                    $where[]      = sprintf('(((`song`.`total_count`/`song`.`total_skip`) * 100) %s ?)', $operator_sql);
                    $parameters[] = $input;
                    break;
                case 'myplayed_times':
                    $my_type = 'song';
                    ...
                    $table['myplayed'] .= (strpos($table['myplayed'], "myplayed_" . $my_type . "_" . $search_user_id))
                        ? ""
                        : "LEFT JOIN " . PlayHistorySubquery::count($my_type, ['stream'], $search_user_id) . " AS `myplayed_" . $my_type . "_" . $search_user_id . "` ON `" . $my_type . "`.`id` = `myplayed_" . $my_type . "_" . $search_user_id . "`.`object_id` AND `myplayed_" . $my_type . "_" . $search_user_id . "`.`object_type` = '" . $my_type . "'";
                ...
                case 'myskipped_times':
                    ...
                    $table['myskipped'] .= (strpos($table['myskipped'], "myskipped_" . $my_type . "_" . $search_user_id))
                        ? ""
                        : "LEFT JOIN " . PlayHistorySubquery::count($my_type, ['skip'], $search_user_id) . " AS `myskipped_" . $my_type . "_" . $search_user_id . "` ON `" . $my_type . "`.`id` = `myskipped_" . $my_type . "_" . $search_user_id . "`.`object_id` AND `myskipped_" . $my_type . "_" . $search_user_id . "`.`object_type` = '" . $my_type . "' ";
                ...
                case 'myplayed_or_skipped_times':
                    ...
                    $table['myplayed_or_skip'] .= (strpos($table['myplayed_or_skip'], "myplayed_or_skip_" . $my_type . "_" . $search_user_id))
                        ? ""
                        : "LEFT JOIN " . PlayHistorySubquery::count($my_type, ['stream', 'skip'], $search_user_id) . " AS `myplayed_or_skip_" . $my_type . "_" . $search_user_id . "` ON `" . $my_type . "`.`id` = `myplayed_or_skip_" . $my_type . "_" . $search_user_id . "`.`object_id` AND `myplayed_or_skip_" . $my_type . "_" . $search_user_id . "`.`object_type` = '" . $my_type . "'";
                ...
                case 'myplayedartist':
                    ...
                    $table['myplayed'] .= (strpos($table['myplayed'], "myplayed_" . $my_type . "_" . $search_user_id))
                        ? ""
                        : "LEFT JOIN " . PlayHistorySubquery::exists($my_type, ['stream'], $search_user_id) . " AS `myplayed_" . $my_type . "_" . $search_user_id . sprintf('` ON `song`.`%s` = `myplayed_', $column) . $my_type . "_" . $search_user_id . "`.`object_id` AND `myplayed_" . $my_type . "_" . $search_user_id . "`.`object_type` = '" . $my_type . "'";
                ...
                case 'favorite':
                    if ($operator_sql === 'NOT SOUNDS LIKE') {
                        ...
                        $where[] = sprintf('`song`.`title` %s ? AND `favorite_song_', $operator_sql) . $search_user_id . "`.`user` = " . $search_user_id . " AND `favorite_song_" . $search_user_id . "`.`object_type` = 'song'";
                    }
                    ...
                    $table['favorite'] .= (strpos($table['favorite'], "favorite_song_" . $search_user_id))
                        ? ""
                ...
                case 'favorite_artist':
                    ...
                    $table['favorite'] .= (strpos($table['favorite'], "favorite_artist_" . $search_user_id))
                        ? ""
                ...
                case 'artistrating':
                    ...
                    $table['rating'] .= (strpos($table['rating'], "rating_" . $my_type . "_" . $search_user_id))
                        ? ""
                        : "LEFT JOIN (SELECT `object_id`, `object_type`, `rating` FROM `rating` WHERE `user` = " . $search_user_id . " AND `object_type` = '" . $my_type . "') AS `rating_" . $my_type . "_" . $search_user_id . "` ON `rating_" . $my_type . "_" . $search_user_id . sprintf('`.`object_id` = `song`.`%s`', $column);
                    break;
                ...
                case 'other_user_artist':
                    ...
                    if ($operator_sql == 'userflag') {
                        ...
                        $table['rating'] .= (strpos($table['rating'], "rating_" . $my_type . "_" . $other_userid))
                            ? ""
                            : "LEFT JOIN `rating` AS `rating_" . $my_type . "_" . $other_userid . "` ON `rating_" . $my_type . "_" . $other_userid . "`.`object_type` = '" . $my_type . "' AND `rating_" . $my_type . "_" . $other_userid . sprintf('`.`object_id` = `song`.`%s` AND `rating_', $column) . $my_type . "_" . $other_userid . sprintf('`.`user` = %s ', $other_userid);
                    }
                ...
                case 'playlist_name':
                    ...
                    break;
                case 'playlist':
                    $where[]      = sprintf("`song`.`id` %s IN (SELECT `object_id` FROM `playlist_data` WHERE `playlist_data`.`playlist` = ? AND `playlist_data`.`object_type` = 'song')", $operator_sql);
                    $parameters[] = $input;
                    break;
                case 'smartplaylist':
                ...
                case 'duplicate_tracks':
                    ...
                    break;
                case 'orphaned_album':
                    $where[] = "(`song`.`album` IN (SELECT `album_id` FROM `album_map` WHERE `album_id` NOT IN (SELECT `id` FROM `album`)) OR `song`.`album` NOT IN (SELECT `id` FROM `album`))";
                    break;
                case 'waveform':
                    $join['song_data'] = true;
                    $operator_sql      = ((int) $operator_sql === 0) ? 'IS NULL' : 'IS NOT NULL';
                ...
                case 'metadata':
                    ...
                    if ($operator_sql === '=' && (string) $input === '') {
                        ...
                        $metadata[$field][] = sprintf('`metadata`.`data` %s ?', $operator_sql);
        ...
        foreach ($metadata as $metadata_field => $metadata_queries) {
            $metadata_sql = sprintf('EXISTS (SELECT NULL FROM `metadata` WHERE `metadata`.`object_id` = `song`.`id` AND `metadata`.`field` = %d AND (', $metadata_field);
            $metadata_sql .= implode(sprintf(' %s ', $sql_logic_operator), $metadata_queries);
            $where[] = $metadata_sql . '))';
        }
