...
<table class="tabledata">
    ...
    ?><a href="<?php echo AmpConfig::get('web_path'); ?>/browse.php?action=tag&type=song"><?php echo T_('Songs'); ?></a><?php
        ...
        ?><a href="<?php echo AmpConfig::get('web_path'); ?>/browse.php?action=tag&type=album"><?php echo T_('Albums'); ?></a><?php
    } else {
        echo T_('Albums');
    } ?></td>
        <td><?php if ($filter_str !== 'artist' && $filter_str !== 'album_artist') {
        ?><a href="<?php echo AmpConfig::get('web_path'); ?>/browse.php?action=tag&type=artist"><?php echo T_('Artists'); ?></a><?php
    } else {
        echo T_('Artists');
    } ?></td>
    <?php if (AmpConfig::get('allow_video') && $videoRepository->getItemCount(Video::class)) { ?>
        <td><?php if ($filter_str != 'video') { ?>
            <a href="<?php echo AmpConfig::get('web_path'); ?>/browse.php?action=tag&type=video"><?php echo T_('Videos'); ?></a><?php
        } else {
            echo T_('Videos');
        } ?></td>
    <?php } ?>
