...
<?php if (User::is_registered() && (AmpConfig::get('ratings'))) { ?>
    <td class="cel_ratings">
        <div class="rating">
            <span id="rating_<?php echo $libitem->getId(); ?>_search" class="cel_rating rating_<?php echo $libitem->getId(); ?>_search">
                <?php echo Rating::show($libitem->getId(), 'search'); ?>
            </span>
            <span id="userflag_<?php echo $libitem->getId(); ?>_search" class="cel_userflag userflag_<?php echo $libitem->getId(); ?>_search">
...
if (Access::check_function(AccessFunctionEnum::FUNCTION_BATCH_DOWNLOAD) && $zipHandler->isZipable('search')) { ?>
                <a class="nohtml" href="<?php echo AmpConfig::get_web_path(); ?>/batch.php?action=search&id=<?php echo $libitem->id; ?>">
