...
<form name="label" method="post" action="<?php echo AmpConfig::get_web_path('/client'); ?>/labels.php?action=add_label">
...
<tr>
    ...
    <tr>
        ...
        <td><input type="text" name="mbid" value="<?php echo scrub_out((string) ($_REQUEST['mbid'] ?? '')); ?>" />
...
<tr>
    ...
    <td>
        <textarea name="summary" cols="44" rows="4"><?php echo scrub_out((string) ($_REQUEST['summary'] ?? '')); ?></textarea>
        <?php echo AmpError::display('summary'); ?>
    </td>
</tr>
<tr>
    <td><?php echo T_('Address'); ?></td>
    <td><input type="text" name="address" value="<?php echo scrub_out((string) ($_REQUEST['address'] ?? '')); ?>" />
        <?php echo AmpError::display('address'); ?>
    </td>
</tr>
    <tr>
        <td><?php echo T_('Country'); ?></td>
        <td><input type="text" name="country" value="<?php echo scrub_out((string) ($_REQUEST['country'] ?? '')); ?>" />
            <?php echo AmpError::display('country'); ?>
        </td>
    </tr>
<tr>
    <td><?php echo T_('E-mail'); ?></td>
    <td><input type="text" name="email" value="<?php echo scrub_out((string) ($_REQUEST['email'] ?? '')); ?>" />
        <?php echo AmpError::display('email'); ?>
    </td>
</tr>
<tr>
    <td><?php echo T_('Website'); ?></td>
    <td><input type="text" name="website" value="<?php echo scrub_out((string) ($_REQUEST['website'] ?? '')); ?>" />
