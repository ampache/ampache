...
* `Album::check()`'s in-process memoization cache keyed on a hardcoded list of columns (name/year/artist/mbid/etc.) but never on catalog, even though the album lookup it caches (`AlbumRepository::findByProperties()`) is itself catalog-scoped by design. A song matching an already-cached album's identity but scanned or uploaded under a different catalog got silently handed that other catalog's album id instead of its own, merging unrelated catalogs' content onto one album and, for a multi-disk album, adding a second `album_disk` row for a disk number it already had (visible as a duplicated `[Disk N]` section). The cache key now always includes catalog, and otherwise tracks whatever `album_grouping_fields` currently matches on (`AlbumRepository::getIdentityColumns()`, now public) instead of a fixed column list, so a field dropped from grouping no longer costs a cache miss for values that would resolve to the same album anyway
* A leftover `album_disk` row left behind by a move (already meant to be cleaned up since 8.0.0) was never actually cleaned up: the query used `NOT IN` against a subquery reading `song`.`disk`, a nullable column, so a single song anywhere with no disk number tainted the comparison with `NULL` and silently kept the check from matching (and deleting) anything, for every album, permanently. Multi-disk albums could show a duplicate `[Disk N]` section indefinitely, surviving even an "Update from tags" retag
...
* A PHP deprecation is logged as `Deprecated` at warning level instead of as an `Error`, so a dependency that has not caught up with the PHP floor no longer fills the log with errors
* A preference reaches the server as the type it declares: an integer preference is an int and a boolean one a bool, instead of the string every value is stored as
* The API3 `stats` method answered with an error instead of a list of random albums when no `limit` was given
* Subsonic
