...
Ampache supports internal Access Control Lists, these are IP/DNS based restrictions on different actions and interactions with Ampache. Access Control Lists are enabled by default (the shipped config sets `access_control = "true"`); set it to false in _/config/ampache.cfg.php_ to turn them off.
...
This is a range of IP addresses represented by a pair of addresses. This does not have to be within a subnet boundary. Both IPv4 and IPv6 addresses are supported (the range endpoints must be the same family).
...
* **API/RPC** - Used to control remote access to your Ampache installation
  * Remote access to the Ampache API
  * Remote catalog sync between Ampache servers (uses the Ampache API; the old XML-RPC transport has been removed)
