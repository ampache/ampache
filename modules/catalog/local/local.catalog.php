 ...
 *
 */
namespace Modules\Catalog\Local;
...
use App\Classes\vaInfo;
use App\Models\Metadata;
use App\Models\Metadata_Field;
...
class Catalog_local extends Catalog
{
    ...
    private $type           = 'local';
    private $description    = 'Local Catalog built on hard drive path';
    ...
    {
        return Schema::hasTable('catalog_local');
    } // is_installed
    ...
    {
        $fields['path']      = array('description' => __('Path'),'type' => 'text');
    ...
    public static function get_from_path($path)
    {
        ...
        $db_results = DB::table('catalog_local')->select('catalog_id', 'path')->get();
        ...
        foreach ($db_results as $result) {
            $catalog_paths[$result->path] = $result->catalog_id;
        }
    ...
    public static function create_type($catalog_id, $data)
    {
        ...
        DB::table('catalog_local')->insert(
            ['path' => $path, 'catalog_id' => $catalog_id]
            );
    ...
    public function add_files($path, $options)
    {
        ...
        if (!is_resource($handle)) {
            ...
            Log::error(sprintf('Error: Unable to open %s', $path));
        ...
        if (!chdir($path)) {
            ...
            Log::error(sprintf('Error: Unable to change to directory %s', $path));
...
//        //debug_event('closedir', "Finished reading $path , closing handle", 5);
    ...
    public function add_file($full_file, $options)
    {
        ...
        if (config('system.no_symlinks') == true) {
        ...
        if (config('catalog.catalog_video_pattern')) {
            $is_video_file = Catalog::is_video_file($full_file);
        }
        
        $is_playlist = false;

        if (($options['parse_playlist'] == true) && config('catalog.catalog_playlist_pattern')) {
        ...
        if ($is_audio_file || $is_video_file || $is_playlist) {
            ...
            if (!$file_size) {
                ...
                /* HINT: FullFile */
                Log::error(sprintf(T_('Error: Unable to get filesize for %s'), $full_file));
            } // file_size check
            ...
            if (!Core::is_readable($full_file)) {
                ...
                /* HINT: FullFile */
                Log::error(sprintf('%s is not readable by ampache'), $full_file);
            ...
            if (function_exists('iconv')) {
                ...
                if (!$convok) {
                    ...
                    /* HINT: FullFile */
                    Log::error(sprintf('%s does not match site charset', $full_file));
    ...
    public function add_to_catalog($options = null)
    {
        ...
        if (!defined('SSE_OUTPUT')) {
            $response = response(printf(__('Starting New Media Search on %s catalog'), "<strong>[ $this->name ]</strong>"), 200);
            echo $response;
            flush();
        }
        ...
        if ($this->gather_types == "podcast") {
            ...
            if (isset($options['parse_playlist']) && count($this->_playlists)) {
    ...
    public function verify_catalog_proc()
    {
        ...
        foreach (array('video', 'song') as $media_type) {
            ...
            DB::table($media_type)->select('id', 'file')->where('catalog', $this->id)->chunk(1000, function ($media_type) {
                foreach ($media_type as $type) {
                    if (!Core::is_readable(Core::conv_lc_file($type->file))) {
                        Log::error(sprintf('%s does not exist or is not readable', $row['file']));
                        //               //debug_event('read', $row['file'] . ' does not exist or is not readable', 5);
                        continue;
                    }
                    
                    $media = new $media_type($type->id);
...
//        //debug_event('verify', "Finished, $total_updated updated in " . $this->name, 5);
    ...
    public function clean_catalog_proc()
    {
        if (!Core::is_readable($this->path)) {
            ...
            Log::error('Catalog Root unreadable, stopping clean');
            Log::error('general');
        ...
        foreach (array('video', 'song') as $media_type) {
            ...
            DB::table($media_type)->select('id', 'file')->where('catalog', $this->id)->chunk(1000, function ($media_type) {
                foreach ($media_type as $type) {
                    $file_info = Core::get_filesize(Core::conv_lc_file($type->file));
                    if (!file_exists(Core::conv_lc_file($results['file'])) || $file_info < 1) {
                        //                //debug_event('clean', 'File not found or empty: ' . $results['file'], 5);
                        Log::error(sprintf('Error File Not Found or 0 Bytes: %s', $results['file']));
            ...
            if ($dead_count >= $total) {
                ...
                Log::error('All files would be removed. Doing nothing');
                continue;
            }
            if ($dead_count) {
                $dead_total += $dead_count;
                DB::table($media_type)->whereIn('id', $dead)->delete();
            }
        }
    ...
    private function insert_local_song($file, $options = array())
    {
        ...
        if (count($this->get_gather_types('music')) > 0) {
            if (config('catalog.catalog_check_duplicate')) {
    ...
    public function insert_local_video($file, $options = array())
    {
        ...
        if ($results['art']) {
            ...
            if (config('catalog.generate_video_preview')) {
     ...
     */
    public function check_local_mp3($full_file)
    {
        ...
        $db_results = DB::table('songs')->select('id')->where('file', $full_file);
