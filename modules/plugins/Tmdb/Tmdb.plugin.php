                ...
                if (!empty($media_info['tvshow'])) {
                    ...
                    if (count($apires['results']) > 0) {
                        ...
                        if ($media_info['Ampache\Model\TVShow_Season']) {
                            $release = $client->getTvSeasonApi()->getSeason($results['tmdb_tvshow_id'], $media_info['Ampache\Model\TVShow_Season']);
                            if ($release['id']) {
                                if ($release['poster_path']) {
                                    $results['tvshow_season_art'] = $imageHelper->getUrl($release['poster_path']);
                                }
                                if ($media_info['Ampache\Model\TVShow_Episode']) {
                                    $release = $client->getTvEpisodeApi()->getEpisode($results['tmdb_tvshow_id'], $media_info['Ampache\Model\TVShow_Season'], $media_info['Ampache\Model\TVShow_Episode']);
                                    if ($release['id']) {
